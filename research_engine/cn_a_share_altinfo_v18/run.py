"""V18 orchestrator. Frozen panel + frozen basics. Resume per hypothesis."""
...
from research_engine.cn_a_share_altinfo_v18 import HOLD_DAYS, RESEARCH, VALIDATION
from research_engine.cn_a_share_altinfo_v18.contract import HYPOTHESES, build_contract
from research_engine.cn_a_share_altinfo_v18.evaluate import apply_gates, decide, run_one, slim_rec
from research_engine.cn_a_share_altinfo_v18.paths import EQUITY, OUT, ensure_v18
from research_engine.cn_a_share_altinfo_v18.signals import build_scores
...
def _progress(stage, **extra):
    ...
    print("V18_PROGRESS", stage, flush=True)
...
def main():
    ...
    for hyp in HYPOTHESES:
        if hyp["id"] in done and "capital" in done[hyp["id"]]:
            print("V18_RESUME", hyp["id"], flush=True)
    ...
    recs, candidates, _failures, _fdr = apply_gates(recs, pack, elig, xok, cache)
    ...
    print("V18_DECISION", decision["OVERALL"], decision["STOP"], "L1", decision["NEW_CANDIDATE"], "IND", decision["NEW_INDEPENDENT_CANDIDATE"], flush=True)
    return decision
