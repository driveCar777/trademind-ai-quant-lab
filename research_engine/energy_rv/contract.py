...
def assert_search_space(space):
    if not space or space.get("discovery_id") != ER_ID:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if int(space.get("seed") or 0) != ER_SEED:
        raise ContractMismatch("CONTRACT_MISMATCH")
    ...
    if list(space.get("hypothesis_ids") or []) != list(ALLOWED_HYPOTHESIS_IDS):
        raise ContractMismatch("CONTRACT_MISMATCH")
    params = space.get("crack_params") or {}
    if int(params.get("ret_lookback") or 0) != int(RET_LOOKBACK):
        raise ContractMismatch("CONTRACT_MISMATCH")
    if space.get("not_eia_weekly") is not True:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if space.get("not_hold_search") is not True:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if space.get("not_oil_sma60") is not True:
        raise ContractMismatch("CONTRACT_MISMATCH")
...
def deny_final_oos(role=None):
    if role is None:
        final_oos_access(reason="energy_rv_v1")
