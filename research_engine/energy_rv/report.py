...
from research_engine.energy_rv import ER_ID
from research_engine.energy_rv.space import canonical_search_space_hash
...
def write_outputs(out_dir, ranking, decision, hashes=None):
    if not os.path.isdir(out_dir):
        os.makedirs(out_dir)
    dump_json(os.path.join(out_dir, "ER_RANKING_V1.json"), ranking)
    dump_json(os.path.join(out_dir, "ER_DECISION_V1.json"), decision)
    md = render_markdown(ranking, decision, hashes=hashes)
    path = os.path.join(out_dir, "ER_REPORT_V1.md")
