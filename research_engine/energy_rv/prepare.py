...
from research_engine.energy_rv import BASKET, PARENTS
from research_engine.energy_rv.contract import assert_search_space
from research_engine.energy_rv.data import load_parents
from research_engine.energy_rv.space import build_search_space
from research_engine.energy_rv.windows import freeze_window, oos_exists
...
def prepare_pack(market_root, out_dir, space=None):
    ...
    if os.path.exists(path):
        ...
        window = freeze_window(book, "ENERGY_RV_ALIGNED_BOOK")
    ...
    dump_json(
        os.path.join(win_dir, "EVENTS_BOOK.json"),
        {
            ...
            "n_crack_cheap_cross": sum(1 for b in book if b.get("is_crack_cheap_cross")),
            "n_crack_rich_cross": sum(1 for b in book if b.get("is_crack_rich_cross")),
        },
    )
    return packed, space
