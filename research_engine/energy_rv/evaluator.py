...
from research_engine.energy_rv import HOLD_BARS, OCCUPANCY_FAIL, ER_BLOCK, ER_BOOT, ER_PERM, ER_SEED
...
from research_engine.energy_rv.windows import same_role_hold
...
def _legs(_aligned, _row, mode):
    if mode == "LONG_PRODUCTS_SHORT_CRUDE":
        return [("GASOLINE", 1), ("HEATOIL", 1), ("OIL", -1)]
    if mode == "LONG_PRODUCTS":
        return [("GASOLINE", 1), ("HEATOIL", 1)]
    if mode == "SHORT_PRODUCTS_LONG_CRUDE":
        return [("GASOLINE", -1), ("HEATOIL", -1), ("OIL", 1)]
    return []
