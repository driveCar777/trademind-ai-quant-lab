#!/usr/bin/env python3
"""Xavier FUT_CFD_LEAD worker. Reads locked space. Does not invent hypotheses."""
...
from research_engine.fut_cfd import DATASET_ID, FUTCFD_ID
from research_engine.fut_cfd.contract import assert_job_contract
...
from research_engine.fut_cfd.space import hypothesis_map
...
def run_job(job, space, market_root, out_dir):
    ...
    payload = {
        ...
        "parent_hashes": packed.get("parent_hashes") or {DATASET_ID: packed.get("sha256")},
...
def main(argv=None):
    ...
    for job in jobs:
        ...
        print("FUTCFD_JOB_DONE", job["job_id"], "%.1f" % payload["elapsed_seconds"])
    dump_json(
        os.path.join(args.out, "NODE_SUMMARY.json"),
        {"node": args.node, "jobs": summary, "host": host_facts()},
    )
    print("FUTCFD_NODE_DONE", args.node, len(jobs))
    return 0
