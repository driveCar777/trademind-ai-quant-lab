...
def futures_hold(bars, signal_t, side, hold_bars=HOLD_BARS):
    ...
    if entry is None or exit_px is None or entry <= 0:
        return None
    floor = ONE_WAY_BP_FLOOR / 10000.0
    spread = _px(bars[entry_i].get("spread"))
    if spread is None:
        one = floor
    elif spread >= 1.0:
        one = (spread * 0.01) / entry
    else:
        one = spread / entry
    if one < floor:
        one = floor
...
def _assert_cfd_knowledge(book, signal_t, root, entry_date):
    session = book[signal_t].get("date")
    if not session or not entry_date:
        raise RuntimeError("KNOWLEDGE_TIME_MISSING")
    assert_feature_before_target(settlement_knowledge_utc(session), entry_date + "T00:00:00Z")
...
def evaluate_hypothesis(
                            ...
                            if steps[0][1]["entry_index"] < next_free:
                                ...
                                if used:
                                    ...
                                    for root, step in used:
                                        ...
                                        _assert_cfd_knowledge(book, i, root, entry_date)
                                        entry_px = step["entry"]
                                        qty = position_qty(
                                            cash / float(len(used)),
                                            0.005,
                                        ...
                                        if qty > 0:
                                            fee = abs(qty * entry_px) * (ONE_WAY_BP_FLOOR / 10000.0)
                                            fee_out = abs(qty * step["exit"]) * (ONE_WAY_BP_FLOOR / 10000.0)
    ...
    return {
        ...
        "hold_bars": 5,
        "one_way_bp_floor": ONE_WAY_BP_FLOOR,
