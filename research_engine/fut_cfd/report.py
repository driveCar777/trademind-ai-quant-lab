"""FUT_CFD_LEAD report and decision. No hash rewrite. No 10% claim."""
from __future__ import print_function

import os

from research_engine.fut_cfd import LOCKED_HASH, FUTCFD_ID
...
def decide(ranking):
    ...
    return {
        ...
        "note": "Do not retune gap/hold. Do not use Ava CFD as spot. Do not read Final OOS.",
    }
...
def write_outputs(out_dir, ranking, decision, hashes=None):
    if not os.path.isdir(out_dir):
        os.makedirs(out_dir)
    dump_json(os.path.join(out_dir, "FUTCFD_RANKING_V1.json"), ranking)
    dump_json(os.path.join(out_dir, "FUTCFD_DECISION_V1.json"), decision)
    md = render_markdown(ranking, decision, hashes=hashes)
    path = os.path.join(out_dir, "FUTCFD_REPORT_V1.md")
