...
from research_engine.time_structure import PARENTS
...
from research_engine.time_structure.space import build_search_space
from research_engine.time_structure.windows import freeze_window, oos_exists
    ...
    for name, row in packed.items():
        ...
        dump_json(
            os.path.join(win_dir, "EVENTS_%s.json" % name),
            {
                ...
                "n_london_open": sum(1 for b in row["bars"] if b.get("is_london_open")),
                "n_ny_fx_open": sum(1 for b in row["bars"] if b.get("is_ny_fx_open")),
            },
        )
    return packed, space
