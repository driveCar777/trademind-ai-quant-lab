...
from research_engine.time_structure import TS_FDR_Q
...
def rank_program(rows):
    ...
    for row in rows:
        ...
        payload.append(item)
    fdr = benjamini_hochberg(pvals, q=TS_FDR_Q)
    i = 0
    while i < len(payload):
    ...
    return {
        "outcome": outcome,
        "fdr": {"q": TS_FDR_Q, "m": fdr.get("m"), "discoveries": fdr.get("discoveries")},
        "gold_pass": gold_pass,
        "oil_pass": oil_pass,
        "single_target": (bool(gold_pass) and not oil_pass) or (bool(oil_pass) and not gold_pass),
        "hypotheses": payload,
        "note": "CANDIDATE needs GOLD and OIL gates plus FDR. Do not invent Tokyo. Not 10%.",
    }
