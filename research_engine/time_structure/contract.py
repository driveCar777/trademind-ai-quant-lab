"""Contract authority. Worker cannot invent sessions or IDs."""
...
def assert_search_space(space):
    if not space or space.get("discovery_id") != TS_ID:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if int(space.get("seed") or 0) != TS_SEED:
        raise ContractMismatch("CONTRACT_MISMATCH")
    ...
    if space.get("not_weekday") is not True:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if space.get("not_institutional_time") is not True:
        raise ContractMismatch("CONTRACT_MISMATCH")
    sessions = space.get("sessions") or {}
    extra = []
...
def deny_final_oos(role=None):
    if role is None:
        final_oos_access(reason="time_structure_v1")
