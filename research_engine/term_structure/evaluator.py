"""Hold=5 next-session open after knowledge. 2 bp one-way. No close fill. No Final OOS."""
...
def futures_hold(bars, signal_t, side, hold_bars=HOLD_BARS):
    ...
    if sched_exit >= len(bars) or entry_i >= len(bars):
        return None
    entry = bars[entry_i].get("open")
    exit_px = bars[sched_exit].get("open")
    if entry in (None,) or exit_px in (None,) or float(entry) <= 0:
        return None
    entry = float(entry)
