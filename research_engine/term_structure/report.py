"""TERM_STRUCTURE report and decision. No hash rewrite. No 10% claim."""
from __future__ import print_function

import os

from research_engine.io_util import dump_json
from research_engine.term_structure import LOCKED_HASH, TS_ID
...
def decide(ranking):
    ...
    return {
        ...
        "slope_change_forbidden": True,
        "hold_change_forbidden": True,
        "strategy_layer_allowed": outcome == "CANDIDATE",
        "note": "Do not retune slope/hold. Do not use Ava CFD as spot. Do not read Final OOS.",
    }
...
def render_markdown(ranking, decision, hashes=None):
    ...
    if hashes:
        ...
        lines.append("")
    lines.append("CAGR >= 10% is not a gate. Not Ava GOLD. Not RSI. Not continuous-only.")
    lines.append("")
    return "\n".join(lines)
...
def write_outputs(out_dir, ranking, decision, hashes=None):
    if not os.path.isdir(out_dir):
        os.makedirs(out_dir)
    dump_json(os.path.join(out_dir, "TS_RANKING_V1.json"), ranking)
    dump_json(os.path.join(out_dir, "TS_DECISION_V1.json"), decision)
    md = render_markdown(ranking, decision, hashes=hashes)
    path = os.path.join(out_dir, "TS_REPORT_V1.md")
