"""Freeze space and the curve-book window before any costed equity."""
...
from research_engine.term_structure.data import load_panel
from research_engine.term_structure.space import build_search_space
...
def prepare_pack(market_root, out_dir, space=None):
    ...
    if os.path.exists(path):
        ...
        window = freeze_window(book, "TSFUT_CURVE_BOOK")
    ...
    dump_json(
        os.path.join(win_dir, "EVENTS_BOOK.json"),
        {
            ...
            "n_backwardation": sum(
                1
                for b in book
                if any((b.get("roots") or {}).get(r) and (b["roots"][r] or {}).get("backwardation") for r in ROOTS)
            ),
        },
    )
    return packed, space
