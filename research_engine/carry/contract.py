...
def assert_search_space(space):
    if not space or space.get("discovery_id") != CARRY_ID:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if int(space.get("seed") or 0) != CARRY_SEED:
        raise ContractMismatch("CONTRACT_MISMATCH")
    ...
    if list(space.get("hypothesis_ids") or []) != list(ALLOWED_HYPOTHESIS_IDS):
        raise ContractMismatch("CONTRACT_MISMATCH")
    params = space.get("carry_params") or {}
    ...
    if space.get("not_zcut_search") is not True:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if space.get("not_v08_fx_price") is not True:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if space.get("not_rates_gold") is not True:
        raise ContractMismatch("CONTRACT_MISMATCH")
    locked = _locked()
...
def assert_job_contract(job, space):
    ...
    if job.get("discovery_id") != CARRY_ID:
        raise ContractMismatch("CONTRACT_MISMATCH")
...
def deny_final_oos(role=None):
    if role is None:
        final_oos_access(reason="carry_v1")
