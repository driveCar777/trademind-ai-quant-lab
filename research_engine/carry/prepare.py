...
from research_engine.carry import PARENTS
from research_engine.carry.contract import assert_search_space
from research_engine.carry.data import load_parents
from research_engine.carry.space import build_search_space
from research_engine.carry.windows import freeze_window, oos_exists
    ...
    for name, row in packed.items():
        ...
        dump_json(
            os.path.join(win_dir, "EVENTS_%s.json" % name),
            {
                ...
                "n_carry_usd_rich_cross": sum(1 for b in row["bars"] if b.get("is_carry_usd_rich_cross")),
                "n_carry_usd_cheap_cross": sum(1 for b in row["bars"] if b.get("is_carry_usd_cheap_cross")),
            },
        )
    return packed, space
