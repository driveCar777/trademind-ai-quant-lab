...
from research_engine.regime_interaction import PARENTS
from research_engine.regime_interaction.contract import assert_search_space
from research_engine.regime_interaction.data import load_parents
from research_engine.regime_interaction.space import build_search_space
from research_engine.regime_interaction.windows import freeze_window, oos_exists
...
def prepare_pack(market_root, out_dir, space=None):
    ...
    for name, row in packed.items():
        ...
        if not oos_exists(window).get("exists"):
        ...
        dump_json(
            os.path.join(win_dir, "EVENTS_%s.json" % name),
            {
                ...
                "n_gold_cheap": sum(1 for b in row["bars"] if b.get("is_gold_cheap")),
                "n_oil_cheap": sum(1 for b in row["bars"] if b.get("is_oil_cheap")),
                "n_joint_vol": sum(1 for b in row["bars"] if b.get("is_joint_vol")),
            },
        )
    return packed, space
