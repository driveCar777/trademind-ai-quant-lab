...
def write_outputs(out_dir, ranking, decision, hashes=None):
    if not os.path.isdir(out_dir):
        os.makedirs(out_dir)
    dump_json(os.path.join(out_dir, "RI_RANKING_V1.json"), ranking)
    dump_json(os.path.join(out_dir, "RI_DECISION_V1.json"), decision)
    lines = [
    ...
    lines.append("")
    path = os.path.join(out_dir, "RI_REPORT_V1.md")
