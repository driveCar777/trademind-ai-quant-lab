from research_engine.regime_interaction import RI_FDR_Q
...
def rank_program(rows):
    ...
    for row in rows:
        ...
        payload.append(item)
    fdr = benjamini_hochberg(pvals, q=RI_FDR_Q)
    i = 0
    while i < len(payload):
    ...
    return {
        "outcome": outcome,
        "fdr": {"q": RI_FDR_Q, "m": fdr.get("m"), "discoveries": fdr.get("discoveries")},
        "gold_pass": gold_pass,
        "oil_pass": oil_pass,
