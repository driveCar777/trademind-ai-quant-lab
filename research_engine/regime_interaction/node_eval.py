...
from research_engine.regime_interaction import RI_ID
from research_engine.regime_interaction.contract import assert_job_contract
...
from research_engine.regime_interaction.space import hypothesis_map
from research_engine.resources import sample_resources
from research_protocol.hashing import canonical_hash


def host_facts():
    info = {"pid": os.getpid(), "python_version": sys.version.split()[0], "cwd": os.getcwd()}
    try:
        import socket
...
def run_job(job, space, market_root, out_dir):
    ...
    for hid in job["hypothesis_ids"]:
        ...
        hyps.append(
            run_hypothesis(
                spec,
                packed,
                seed=int(job.get("seed") or 20260828),
    ...
    payload = {
        ...
        "parent_hashes": dict((k, packed[k]["sha256"]) for k in packed),
        "window_hashes": dict((k, packed[k]["window"]["window_hash"]) for k in packed),
...
def main(argv=None):
    ...
    for job in bundle.get("jobs") or []:
        ...
        summary.append({"job_id": job["job_id"], "content_hash": payload["content_hash"], "elapsed_seconds": payload["elapsed_seconds"]})
        print("RI_JOB_DONE", job["job_id"], "%.1f" % payload["elapsed_seconds"])
    dump_json(os.path.join(args.out, "NODE_SUMMARY.json"), {"node": args.node, "jobs": summary, "host": host_facts()})
    print("RI_NODE_DONE", args.node, len(summary))
    return 0
