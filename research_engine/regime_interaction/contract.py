...
from research_engine.regime_interaction.space import canonical_search_space_hash


def assert_search_space(space):
    if not space or space.get("discovery_id") != RI_ID:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if int(space.get("seed") or 0) != RI_SEED:
        raise ContractMismatch("CONTRACT_MISMATCH")
    ...
    if int(space.get("hold_bars") or 0) != HOLD_BARS:
        raise ContractMismatch("CONTRACT_MISMATCH")
    rel = space.get("relative_vol") or {}
    if int(rel.get("rv_n") or 0) != RV_N:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if int(rel.get("self_n") or 0) != SELF_N:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if space.get("not_v09") is not True or space.get("not_v08") is not True or space.get("not_v091") is not True:
        raise ContractMismatch("CONTRACT_MISMATCH")
