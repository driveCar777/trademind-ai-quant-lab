...
from research_engine.implied_vol.space import canonical_search_space_hash
...
def assert_search_space(space):
    if not space or space.get("discovery_id") != IV_ID:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if int(space.get("seed") or 0) != IV_SEED:
        raise ContractMismatch("CONTRACT_MISMATCH")
...
def deny_final_oos(role=None):
    if role is None:
        final_oos_access(reason="implied_vol_v1")
