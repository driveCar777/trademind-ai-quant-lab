...
from research_engine.implied_vol import PARENTS
from research_engine.implied_vol.contract import assert_search_space
from research_engine.implied_vol.data import load_parents
from research_engine.implied_vol.space import build_search_space
from research_engine.implied_vol.windows import freeze_window, oos_exists
    ...
    for name, row in packed.items():
        ...
        dump_json(
            os.path.join(win_dir, "EVENTS_%s.json" % name),
            {
                ...
                "n_gvz_z_cross": sum(1 for b in row["bars"] if b.get("is_gvz_z_cross")),
                "n_ovx_z_cross": sum(1 for b in row["bars"] if b.get("is_ovx_z_cross")),
                "n_gvz_vrp_rich_cross": sum(1 for b in row["bars"] if b.get("is_gvz_vrp_rich_cross")),
            },
        )
    return packed, space
