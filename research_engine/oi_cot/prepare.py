...
from research_engine.oi_cot import CURVE_ID, ROOTS
from research_engine.oi_cot.contract import assert_search_space
from research_engine.oi_cot.data import load_panel
from research_engine.oi_cot.space import build_search_space
...
def prepare_pack(market_root, out_dir, space=None):
    ...
    if os.path.exists(path):
        ...
        window = freeze_window(book, "OICOT_BOOK")
    ...
    dump_json(
        os.path.join(win_dir, "EVENTS_BOOK.json"),
        {
            ...
            "n_build_align": sum(
                1
                for b in book
                if any((b.get("roots") or {}).get(r) and (b["roots"][r] or {}).get("is_build_align") for r in ROOTS)
            ),
            "n_speed_gap": sum(
                1
                for b in book
                if any((b.get("roots") or {}).get(r) and (b["roots"][r] or {}).get("is_speed_gap") for r in ROOTS)
            ),
            "n_unwind_align": sum(
                1
                for b in book
                if any((b.get("roots") or {}).get(r) and (b["roots"][r] or {}).get("is_unwind_align") for r in ROOTS)
            ),
        },
    )
    return packed, space
