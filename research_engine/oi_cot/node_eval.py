...
from research_engine.oi_cot import DATASET_ID, OICOT_ID
from research_engine.oi_cot.contract import assert_job_contract
...
from research_engine.oi_cot.space import hypothesis_map
...
def main(argv=None):
    ...
    for job in jobs:
        ...
        print("OICOT_JOB_DONE", job["job_id"], "%.1f" % payload["elapsed_seconds"])
    dump_json(
        os.path.join(args.out, "NODE_SUMMARY.json"),
        {"node": args.node, "jobs": summary, "host": host_facts()},
    )
    print("OICOT_NODE_DONE", args.node, len(jobs))
    return 0
