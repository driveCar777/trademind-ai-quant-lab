#!/usr/bin/env python3
"""Xavier IT worker. Reads locked space. Does not invent hypotheses."""
...
from research_engine.institutional_time import IT_ID
from research_engine.institutional_time.contract import assert_job_contract
...
from research_engine.institutional_time.space import hypothesis_map
...
def main(argv=None):
    ...
    for job in jobs:
        ...
        print("IT_JOB_DONE", job["job_id"], "%.1f" % payload["elapsed_seconds"])
    dump_json(
        os.path.join(args.out, "NODE_SUMMARY.json"),
        {"node": args.node, "jobs": summary, "host": host_facts()},
    )
    print("IT_NODE_DONE", args.node, len(jobs))
    return 0
