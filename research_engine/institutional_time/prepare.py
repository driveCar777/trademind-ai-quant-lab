...
from research_engine.institutional_time import PARENTS
...
from research_engine.institutional_time.space import build_search_space
from research_engine.institutional_time.windows import freeze_window, oos_exists
...
def prepare_pack(market_root, out_dir, space=None):
    ...
    for name, row in packed.items():
        ...
        while i < n:
            ...
            i += 1
        row["window"] = window
        dump_json(os.path.join(win_dir, "EVENTS_%s.json" % name), {
            "dataset_id": row["dataset_id"],
            "n_month_end": sum(1 for b in row["bars"] if b.get("is_month_end")),
            "n_month_start": sum(1 for b in row["bars"] if b.get("is_month_start")),
        })
    return packed, space
