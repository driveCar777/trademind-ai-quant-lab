...
from research_engine.alt_market_structure.contract import assert_job_contract
...
from research_engine.alt_market_structure.space import hypothesis_map
...
def main(argv=None):
    ...
    for job in bundle.get("jobs") or []:
        ...
        print("AMS_JOB_DONE", job["job_id"], "%.1f" % payload["elapsed_seconds"])
    dump_json(os.path.join(args.out, "NODE_SUMMARY.json"), {"node": args.node, "jobs": summary, "host": host_facts()})
    print("AMS_NODE_DONE", args.node, len(summary))
    return 0
