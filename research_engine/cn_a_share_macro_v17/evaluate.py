"""V17 dual-book evaluation. MEAN_FORWARD_RETURN is not CAGR."""
...
from research_engine.cn_a_share_macro_v17 import DENIED, FDR_Q, HOLD_DAYS, RESEARCH, SAME_CLUSTER_CORR, VALIDATION
from research_engine.cn_a_share_macro_v17.contract import HYPOTHESES
from research_engine.cn_a_share_macro_v17.paths import EQUITY, OUT, TRADES, ensure_v17
from research_engine.cn_a_share_macro_v17.signals import score_matrix


SIGN = {
    "NEG_BETA_TIMES_SHOCK": -1.0,
    "BETA_TIMES_SHOCK": 1.0,
}
...
def coverage(shock, dates, start, end):
    i0 = dates.index(start)
    i1 = dates.index(end)
    sl = shock[i0 : i1 + 1]
    n = int(sl.size)
    k = int(np.sum(np.isfinite(sl)))
    return {"n_days": n, "n_finite": k, "rate": None if n == 0 else float(k) / float(n)}


def run_one(pack, elig, xok, ew, shocks, hyp, legacy):
    hid = hyp["id"]
    shock = shocks[hyp["macro"]]
    sign = SIGN[hyp["signal"]]
    print("V17", hid, "SCORE", flush=True)
    scores = score_matrix(pack, shock, hyp["lookback"], sign)
    print("V17", hid, "PRED", flush=True)
    ...
    denied = [r for r in pred if DENIED[0] <= r["date"] <= DENIED[1]]
    if denied:
    ...
    print("V17", hid, "CAPITAL", flush=True)
    ...
    rec = {
        ...
        "coverage": {
            "research": coverage(shock, pack["dates"], RESEARCH[0], RESEARCH[1]),
            "validation": coverage(shock, pack["dates"], VALIDATION[0], VALIDATION[1]),
        },
        "predictive": {
            "research": dict(
...
def apply_gates(recs, pack, elig, xok, shocks):
    ...
    for i, r in enumerate(recs):
        ...
        if ok:
            ...
            print("V17_STRESS", r["id"], flush=True)
            scores = score_matrix(pack, shocks[hyp["macro"]], hyp["lookback"], SIGN[hyp["signal"]])
            stress = {}
...
def decide(candidates, ran):
    ...
    if independent:
        ...
        overall = "A_SHARE_MACRO_INFORMATION_V1_NO_CANDIDATE"
        nxt = "MACRO_NO_CANDIDATE"
        stop = "STOP_B_FAMILY"
    else:
