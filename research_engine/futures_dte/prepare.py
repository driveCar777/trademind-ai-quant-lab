...
from research_engine.futures_dte.data import load_panel
from research_engine.futures_dte.space import build_search_space
...
def prepare_pack(market_root, out_dir, space=None):
    ...
    if os.path.exists(path):
        ...
        window = freeze_window(book, "FUTDTE_DTE_BOOK")
    ...
    dump_json(
        os.path.join(win_dir, "EVENTS_BOOK.json"),
        {
            ...
            "n_near_expiry": sum(
                1
                for b in book
                if any((b.get("roots") or {}).get(r) and (b["roots"][r] or {}).get("is_near_expiry") for r in ROOTS)
            ),
            "n_front_roll": sum(
                1
                for b in book
                if any((b.get("roots") or {}).get(r) and (b["roots"][r] or {}).get("is_front_roll") for r in ROOTS)
            ),
            "n_post_roll": sum(
                1
                for b in book
                if any((b.get("roots") or {}).get(r) and (b["roots"][r] or {}).get("is_post_roll") for r in ROOTS)
            ),
        },
    )
    return packed, space
