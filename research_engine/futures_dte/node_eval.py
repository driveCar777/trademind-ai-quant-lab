#!/usr/bin/env python3
"""Xavier DTE_ROLL_WINDOW worker. Reads locked space. Does not invent hypotheses."""
...
from research_engine.futures_dte import DATASET_ID, DTE_ID
from research_engine.futures_dte.contract import assert_job_contract
...
from research_engine.futures_dte.space import hypothesis_map
...
def main(argv=None):
    ...
    for job in jobs:
        ...
        print("FUTDTE_JOB_DONE", job["job_id"], "%.1f" % payload["elapsed_seconds"])
    dump_json(
        os.path.join(args.out, "NODE_SUMMARY.json"),
        {"node": args.node, "jobs": summary, "host": host_facts()},
    )
    print("FUTDTE_NODE_DONE", args.node, len(jobs))
    return 0
