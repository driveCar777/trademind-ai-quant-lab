#!/usr/bin/env python3
"""Xavier BREADTH worker. Reads locked space. Does not invent hypotheses."""
...
from research_engine.breadth import BASKET, BR_ID
from research_engine.breadth.contract import assert_job_contract
...
from research_engine.breadth.space import hypothesis_map
...
def main(argv=None):
    ...
    for job in jobs:
        ...
        print("BR_JOB_DONE", job["job_id"], "%.1f" % payload["elapsed_seconds"])
    dump_json(
        os.path.join(args.out, "NODE_SUMMARY.json"),
        {"node": args.node, "jobs": summary, "host": host_facts()},
    )
    print("BR_NODE_DONE", args.node, len(jobs))
    return 0
