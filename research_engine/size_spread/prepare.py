...
from research_engine.size_spread import BASKET, PARENTS
from research_engine.size_spread.contract import assert_search_space
from research_engine.size_spread.data import load_parents
from research_engine.size_spread.space import build_search_space
from research_engine.size_spread.windows import freeze_window, oos_exists
...
def prepare_pack(market_root, out_dir, space=None):
    ...
    if os.path.exists(path):
        ...
        window = freeze_window(book, "SIZE_ALIGNED_BOOK")
    ...
    dump_json(
        os.path.join(win_dir, "EVENTS_BOOK.json"),
        {
            ...
            "n_lead": sum(1 for b in book if b.get("is_size_lead")),
            "n_lag": sum(1 for b in book if b.get("is_size_lag")),
        },
    )
    return packed, space
