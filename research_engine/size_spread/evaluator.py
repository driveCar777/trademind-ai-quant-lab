"""Breadth book. Hold=5 NEXT_BAR_OPEN. Overlap skip. No Final OOS."""
...
from research_engine.size_spread import HOLD_BARS, OCCUPANCY_FAIL, SZ_BLOCK, SZ_BOOT, SZ_PERM, SZ_SEED
...
from research_engine.size_spread.windows import same_role_hold
...
def _legs(_aligned, _row, mode):
    ...
    if mode == "GOLD_SHORT":
        return [("GOLD", -1)]
    if mode == "SHORT_SMALL_LONG_LARGE":
        return [("US2000", -1), ("US500", 1)]
    return []
...
def evaluate_hypothesis(
    ...
    event = spec["event"]
    mode = spec.get("side_mode") or "GOLD_LONG"
    ...
    return {
        ...
        "target_asset": spec.get("target_asset") or "GOLD",
        "event": event,
        "side_mode": mode,
