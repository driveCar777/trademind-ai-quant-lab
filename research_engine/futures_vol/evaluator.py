...
def _assert_vol_knowledge(book, signal_t, root, entry_date):
    feat = ((book[signal_t].get("roots") or {}).get(root)) or {}
    lagged = feat.get("lagged_session_date")
    if not lagged or not entry_date:
        raise RuntimeError("KNOWLEDGE_TIME_MISSING")
    assert_feature_before_target(oi_knowledge_utc(lagged), entry_date + "T00:00:00Z")
                            ...
                            if steps[0][1]["entry_index"] < next_free:
                                ...
                                if used:
                                    ...
                                    for root, step in used:
                                        ...
                                        _assert_vol_knowledge(book, i, root, entry_date)
                                        entry_px = step["entry"]
                                        qty = position_qty(
                                            cash / float(len(used)),
                                            0.005,
