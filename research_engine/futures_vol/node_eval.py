#!/usr/bin/env python3
"""Xavier VOLUME_PRICE_FLOW worker. Reads locked space. Does not invent hypotheses."""
...
from research_engine.futures_vol import DATASET_ID, VOL_ID
from research_engine.futures_vol.contract import assert_job_contract
...
from research_engine.futures_vol.space import hypothesis_map
...
def main(argv=None):
    ...
    for job in jobs:
        ...
        print("FUTVOL_JOB_DONE", job["job_id"], "%.1f" % payload["elapsed_seconds"])
    dump_json(
        os.path.join(args.out, "NODE_SUMMARY.json"),
        {"node": args.node, "jobs": summary, "host": host_facts()},
    )
    print("FUTVOL_NODE_DONE", args.node, len(jobs))
    return 0
