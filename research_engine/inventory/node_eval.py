...
from research_engine.inventory import INV_ID
from research_engine.inventory.contract import assert_job_contract
...
from research_engine.inventory.space import hypothesis_map
...
def main(argv=None):
    ...
    for job in jobs:
        ...
        print("INV_JOB_DONE", job["job_id"], "%.1f" % payload["elapsed_seconds"])
    dump_json(
        os.path.join(args.out, "NODE_SUMMARY.json"),
        {"node": args.node, "jobs": summary, "host": host_facts()},
    )
    print("INV_NODE_DONE", args.node, len(jobs))
    return 0
