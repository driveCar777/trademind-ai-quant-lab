...
from research_engine.inventory.space import canonical_search_space_hash
...
def assert_search_space(space):
    if not space or space.get("discovery_id") != INV_ID:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if int(space.get("seed") or 0) != INV_SEED:
        raise ContractMismatch("CONTRACT_MISMATCH")
    ...
    if list(space.get("hypothesis_ids") or []) != list(ALLOWED_HYPOTHESIS_IDS):
        raise ContractMismatch("CONTRACT_MISMATCH")
    params = space.get("inv_params") or {}
...
def assert_job_contract(job, space):
    ...
    if job.get("discovery_id") != INV_ID:
        raise ContractMismatch("CONTRACT_MISMATCH")
