...
from research_engine.inventory import PARENTS
from research_engine.inventory.contract import assert_search_space
from research_engine.inventory.data import load_parents
from research_engine.inventory.space import build_search_space
from research_engine.inventory.windows import freeze_window, oos_exists
    ...
    for name, row in packed.items():
        ...
        dump_json(
            os.path.join(win_dir, "EVENTS_%s.json" % name),
            {
                ...
                "n_inv_draw_cross": sum(1 for b in row["bars"] if b.get("is_inv_draw_cross")),
                "n_inv_build_cross": sum(1 for b in row["bars"] if b.get("is_inv_build_cross")),
            },
        )
    return packed, space
