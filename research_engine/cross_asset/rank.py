"""Program labels. Gates locked before the run. m=3."""
from __future__ import print_function

from research_engine.cross_asset import CROSS_FDR_Q
...
def rank_program(rows):
    ...
    for row in rows:
        ...
        payload.append(item)
    fdr = benjamini_hochberg(pvals, q=CROSS_FDR_Q)
    i = 0
    while i < len(payload):
    ...
    return {
        "outcome": outcome,
        "fdr": {"q": CROSS_FDR_Q, "m": fdr.get("m"), "discoveries": fdr.get("discoveries")},
        "gold_pass": gold_pass,
        "oil_pass": oil_pass,
        "hypotheses": payload,
        "note": "CANDIDATE needs GOLD and OIL gates plus FDR. Not 10%.",
    }
