...
from research_engine.hot_mt5_gold_h1_v2.engine import run
from research_engine.hot_mt5_gold_h1_v2.paths import RES
...
def main(argv=None):
    ...
    if (RES / "READ.json").is_file() and not args.force:
        print("READ exists — refuse second train of HOT_MT5_GOLD_H1_V2_SESSION")
        return 0
    summary = run()
    ...
    for name, book in summary["books"].items():
        ...
        print("%s %s long TWR=%s hit=%s payoff=%s cover=%s cash_days=%s | val n=%s TWR=%s t=%s cover=%s | month TWR=%s | %s" % (
            name, book["verdict"],
    ...
    print("SUMMARY viable", summary["n_viable"], "/ 2 candidate=false H1 session same-day")
    return 0
