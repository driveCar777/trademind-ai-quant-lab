"""Lag OI events by one session so knowledge T+1 21:00Z is respected."""
from __future__ import print_function

from research_engine.futures_oi import ROOTS
...
def tag_book(rows):
    ...
    for session in dates:
        ...
        for root in ROOTS:
            ...
            feat["is_new_longs"] = _flag(lagged, "new_longs")
            feat["is_short_cover"] = _flag(lagged, "short_cover")
            feat["is_new_shorts"] = _flag(lagged, "new_shorts")
            roots[root] = feat
            aligned[root].append(
                {
                    "date": session,
