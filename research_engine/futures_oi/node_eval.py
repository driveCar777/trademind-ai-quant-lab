...
from research_engine.futures_oi import DATASET_ID, OI_ID
from research_engine.futures_oi.contract import assert_job_contract
...
from research_engine.futures_oi.space import hypothesis_map
...
def main(argv=None):
    ...
    for job in jobs:
        ...
        print("FUTOI_JOB_DONE", job["job_id"], "%.1f" % payload["elapsed_seconds"])
    dump_json(
        os.path.join(args.out, "NODE_SUMMARY.json"),
        {"node": args.node, "jobs": summary, "host": host_facts()},
    )
    print("FUTOI_NODE_DONE", args.node, len(jobs))
    return 0
