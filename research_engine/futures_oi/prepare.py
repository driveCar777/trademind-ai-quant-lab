...
from research_engine.futures_oi import DATASET_ID, ROOTS
from research_engine.futures_oi.contract import assert_search_space
from research_engine.futures_oi.data import load_panel
from research_engine.futures_oi.space import build_search_space
...
def prepare_pack(market_root, out_dir, space=None):
    ...
    if os.path.exists(path):
        ...
        window = freeze_window(book, "FUTOI_OI_BOOK")
    ...
    dump_json(
        os.path.join(win_dir, "EVENTS_BOOK.json"),
        {
            ...
            "n_new_longs": sum(
                1
                for b in book
                if any((b.get("roots") or {}).get(r) and (b["roots"][r] or {}).get("is_new_longs") for r in ROOTS)
            ),
            "n_short_cover": sum(
                1
                for b in book
                if any((b.get("roots") or {}).get(r) and (b["roots"][r] or {}).get("is_short_cover") for r in ROOTS)
            ),
            "n_new_shorts": sum(
                1
                for b in book
                if any((b.get("roots") or {}).get(r) and (b["roots"][r] or {}).get("is_new_shorts") for r in ROOTS)
            ),
        },
    )
    return packed, space
