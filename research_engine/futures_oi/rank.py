...
from research_engine.futures_oi import OI_FDR_Q
...
def rank_program(rows):
    ...
    for row in rows:
        ...
        payload.append(item)
    fdr = benjamini_hochberg(pvals, q=OI_FDR_Q)
    i = 0
    passed = []
    fdr_pass = []
    while i < len(payload):
    ...
    return {
        "outcome": outcome,
        "fdr": {"q": OI_FDR_Q, "m": fdr.get("m"), "discoveries": fdr.get("discoveries")},
        "book_pass": passed,
        "fdr_pass": fdr_pass,
        "hypotheses": payload,
        "note": "CANDIDATE needs two book hyps plus FDR. Not 10%. Do not retune OI.",
    }
