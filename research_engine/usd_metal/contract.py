...
from research_engine.usd_metal.space import canonical_search_space_hash
...
def assert_search_space(space):
    if not space or space.get("discovery_id") != UM_ID:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if int(space.get("seed") or 0) != UM_SEED:
        raise ContractMismatch("CONTRACT_MISMATCH")
    ...
    if list(space.get("hypothesis_ids") or []) != list(ALLOWED_HYPOTHESIS_IDS):
        raise ContractMismatch("CONTRACT_MISMATCH")
    params = space.get("um_params") or {}
    if float(params.get("z_cut") or 0) != float(Z_CUT):
        raise ContractMismatch("CONTRACT_MISMATCH")
    if int(params.get("z_lookback") or 0) != int(Z_LOOKBACK):
        raise ContractMismatch("CONTRACT_MISMATCH")
    if space.get("not_v08_fx_return") is not True:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if space.get("not_cross_metal_ratio") is not True:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if space.get("not_zcut_search") is not True:
        raise ContractMismatch("CONTRACT_MISMATCH")
    locked = _locked()
...
def deny_final_oos(role=None):
    if role is None:
        final_oos_access(reason="usd_metal_v1")
