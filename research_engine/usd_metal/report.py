...
def render_markdown(ranking, decision, hashes=None):
    ...
    if hashes:
        ...
        lines.append("")
    lines.append("CAGR >= 10% is not a gate. Not V0.8 pair return. Not gold-silver ratio. Not a z_cut search.")
    lines.append("")
    return "\n".join(lines)
...
def write_outputs(out_dir, ranking, decision, hashes=None):
    if not os.path.isdir(out_dir):
        os.makedirs(out_dir)
    dump_json(os.path.join(out_dir, "UM_RANKING_V1.json"), ranking)
    dump_json(os.path.join(out_dir, "UM_DECISION_V1.json"), decision)
    md = render_markdown(ranking, decision, hashes=hashes)
    path = os.path.join(out_dir, "UM_REPORT_V1.md")
