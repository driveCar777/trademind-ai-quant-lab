...
from research_engine.usd_metal import PARENTS, TARGETS
from research_engine.usd_metal.contract import assert_search_space
from research_engine.usd_metal.data import load_parents
from research_engine.usd_metal.space import build_search_space
from research_engine.usd_metal.windows import freeze_window, oos_exists
...
def prepare_pack(market_root, out_dir, space=None):
    ...
    for name, row in packed.items():
        if name.startswith("_") or name not in TARGETS:
            continue
        ...
        dump_json(
            os.path.join(win_dir, "EVENTS_%s.json" % name),
            {
                ...
                "n_dxy_up_cross": sum(1 for b in row["bars"] if b.get("is_dxy_up_cross")),
                "n_dxy_down_cross": sum(1 for b in row["bars"] if b.get("is_dxy_down_cross")),
            },
        )
    return packed, space
