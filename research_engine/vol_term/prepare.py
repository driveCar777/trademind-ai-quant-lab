...
from research_engine.vol_term import BASKET, PARENTS
from research_engine.vol_term.contract import assert_search_space
from research_engine.vol_term.data import load_parents
from research_engine.vol_term.space import build_search_space
from research_engine.vol_term.windows import freeze_window, oos_exists
...
def prepare_pack(market_root, out_dir, space=None):
    ...
    if os.path.exists(path):
        ...
        window = freeze_window(book, "VOL_TERM_ALIGNED_BOOK")
    ...
    dump_json(
        os.path.join(win_dir, "EVENTS_BOOK.json"),
        {
            ...
            "n_vol_steep_cross": sum(1 for b in book if b.get("is_vol_steep_cross")),
            "n_vol_flat_cross": sum(1 for b in book if b.get("is_vol_flat_cross")),
        },
    )
    return packed, space
