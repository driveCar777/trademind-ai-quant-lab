#!/usr/bin/env python3
"""Xavier XS_REV worker. Reads locked space. Does not invent hypotheses."""
...
from research_engine.vol_term import BASKET, VT_ID
from research_engine.vol_term.contract import assert_job_contract
...
from research_engine.vol_term.space import hypothesis_map
...
def _parent_hashes(packed):
    out = {}
    for name in list(BASKET) + ["GOLD_H1"]:
        row = packed.get(name) or {}
        if row.get("sha256"):
            out[name] = row["sha256"]
    return out
...
def run_job(job, space, market_root, out_dir):
    ...
    payload = {
        ...
        "parent_hashes": _parent_hashes(packed),
...
def main(argv=None):
    ...
    for job in jobs:
        ...
        print("VT_JOB_DONE", job["job_id"], "%.1f" % payload["elapsed_seconds"])
    dump_json(
        os.path.join(args.out, "NODE_SUMMARY.json"),
        {"node": args.node, "jobs": summary, "host": host_facts()},
    )
    print("VT_NODE_DONE", args.node, len(jobs))
    return 0
