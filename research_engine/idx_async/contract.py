...
def assert_search_space(space):
    if not space or space.get("discovery_id") != IA_ID:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if int(space.get("seed") or 0) != IA_SEED:
        raise ContractMismatch("CONTRACT_MISMATCH")
    ...
    if list(space.get("hypothesis_ids") or []) != list(ALLOWED_HYPOTHESIS_IDS):
        raise ContractMismatch("CONTRACT_MISMATCH")
    params = space.get("idx_params") or {}
    if int(params.get("pct_lookback") or 0) != int(PCT_LOOKBACK):
        raise ContractMismatch("CONTRACT_MISMATCH")
    if space.get("not_v08_fx_return") is not True:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if space.get("not_hold_search") is not True:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if space.get("not_xs_rank_book") is not True:
        raise ContractMismatch("CONTRACT_MISMATCH")
...
def deny_final_oos(role=None):
    if role is None:
        final_oos_access(reason="idx_async_v1")
