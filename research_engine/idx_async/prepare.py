...
from research_engine.idx_async import BASKET, PARENTS
from research_engine.idx_async.contract import assert_search_space
from research_engine.idx_async.data import load_parents
from research_engine.idx_async.space import build_search_space
from research_engine.idx_async.windows import freeze_window, oos_exists
...
def prepare_pack(market_root, out_dir, space=None):
    ...
    if os.path.exists(path):
        ...
        window = freeze_window(book, "IDX_ASYNC_ALIGNED_BOOK")
    ...
    dump_json(
        os.path.join(win_dir, "EVENTS_BOOK.json"),
        {
            ...
            "n_ger_up_cross": sum(1 for b in book if b.get("is_ger_up_cross")),
            "n_ger_down_cross": sum(1 for b in book if b.get("is_ger_down_cross")),
        },
    )
    return packed, space
