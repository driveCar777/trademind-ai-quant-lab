...
from research_engine.corr_shock import BASKET, PARENTS
from research_engine.corr_shock.contract import assert_search_space
from research_engine.corr_shock.data import load_parents
from research_engine.corr_shock.space import build_search_space
from research_engine.corr_shock.windows import freeze_window, oos_exists
...
def prepare_pack(market_root, out_dir, space=None):
    ...
    if os.path.exists(path):
        ...
        window = freeze_window(book, "CORR_SHOCK_ALIGNED_BOOK")
    ...
    dump_json(
        os.path.join(win_dir, "EVENTS_BOOK.json"),
        {
            ...
            "n_corr_break_cross": sum(1 for b in book if b.get("is_corr_break_cross")),
            "n_corr_spike_cross": sum(1 for b in book if b.get("is_corr_spike_cross")),
        },
    )
    return packed, space
