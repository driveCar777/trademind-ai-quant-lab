...
def assert_search_space(space):
    if not space or space.get("discovery_id") != CS_ID:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if int(space.get("seed") or 0) != CS_SEED:
        raise ContractMismatch("CONTRACT_MISMATCH")
    ...
    if list(space.get("hypothesis_ids") or []) != list(ALLOWED_HYPOTHESIS_IDS):
        raise ContractMismatch("CONTRACT_MISMATCH")
    params = space.get("corr_params") or {}
    if int(params.get("corr_lookback") or 0) != int(CORR_LOOKBACK):
        raise ContractMismatch("CONTRACT_MISMATCH")
    if space.get("not_dxy_z") is not True:
        raise ContractMismatch("CONTRACT_MISMATCH")
...
def deny_final_oos(role=None):
    if role is None:
        final_oos_access(reason="corr_shock_v1")
