"""XS_REV report and decision. No hash rewrite. No 10% claim."""
from __future__ import print_function

import os

from research_engine.corr_shock import CS_ID
from research_engine.corr_shock.space import canonical_search_space_hash
...
def decide(ranking):
    ...
    return {
        ...
        "note": "Do not retune corr lookback or percentiles. Do not flip signs. Do not read Final OOS.",
    }
...
def render_markdown(ranking, decision, hashes=None):
    ...
    if hashes:
        ...
        lines.append("")
    lines.append("CAGR >= 10% is not a gate. Not single-name momentum. Not V0.8 pair return. Not gold-silver / DXY z.")
    lines.append("")
    return "\n".join(lines)
...
def write_outputs(out_dir, ranking, decision, hashes=None):
    if not os.path.isdir(out_dir):
        os.makedirs(out_dir)
    dump_json(os.path.join(out_dir, "CS_RANKING_V1.json"), ranking)
    dump_json(os.path.join(out_dir, "CS_DECISION_V1.json"), decision)
    md = render_markdown(ranking, decision, hashes=hashes)
    path = os.path.join(out_dir, "CS_REPORT_V1.md")
