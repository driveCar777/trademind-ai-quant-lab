...
from research_engine.corr_shock import BASKET, CS_ID
from research_engine.corr_shock.contract import assert_job_contract
...
from research_engine.corr_shock.space import hypothesis_map
...
def main(argv=None):
    ...
    for job in jobs:
        ...
        print("CS_JOB_DONE", job["job_id"], "%.1f" % payload["elapsed_seconds"])
    dump_json(
        os.path.join(args.out, "NODE_SUMMARY.json"),
        {"node": args.node, "jobs": summary, "host": host_facts()},
    )
    print("CS_NODE_DONE", args.node, len(jobs))
    return 0
