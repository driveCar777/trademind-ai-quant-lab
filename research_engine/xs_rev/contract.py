...
def assert_search_space(space):
    if not space or space.get("discovery_id") != XS_ID:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if int(space.get("seed") or 0) != XS_SEED:
        raise ContractMismatch("CONTRACT_MISMATCH")
    ...
    if list(space.get("hypothesis_ids") or []) != list(ALLOWED_HYPOTHESIS_IDS):
        raise ContractMismatch("CONTRACT_MISMATCH")
    params = space.get("xs_params") or {}
    if int(params.get("lookback") or 0) != int(LOOKBACK):
        raise ContractMismatch("CONTRACT_MISMATCH")
    if space.get("not_single_asset_momentum") is not True:
        raise ContractMismatch("CONTRACT_MISMATCH")
...
def deny_final_oos(role=None):
    if role is None:
        final_oos_access(reason="xs_rev_v1")
