...
from research_engine.xs_rev import BASKET, PARENTS
from research_engine.xs_rev.contract import assert_search_space
from research_engine.xs_rev.data import load_parents
from research_engine.xs_rev.space import build_search_space
from research_engine.xs_rev.windows import freeze_window, oos_exists
...
def prepare_pack(market_root, out_dir, space=None):
    ...
    if os.path.exists(path):
        ...
        window = freeze_window(book, "XS_REV_ALIGNED_BOOK")
    ...
    dump_json(
        os.path.join(win_dir, "EVENTS_BOOK.json"),
        {
            ...
            "n_xs_rebal": sum(1 for b in book if b.get("is_xs_rebal")),
            "n_xs_rebal_wide": sum(1 for b in book if b.get("is_xs_rebal_wide")),
        },
    )
    return packed, space
