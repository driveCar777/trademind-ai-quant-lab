...
from research_engine.xs_rev import BASKET, XS_ID
from research_engine.xs_rev.contract import assert_job_contract
...
from research_engine.xs_rev.space import hypothesis_map
...
def main(argv=None):
    ...
    for job in jobs:
        ...
        print("XS_JOB_DONE", job["job_id"], "%.1f" % payload["elapsed_seconds"])
    dump_json(
        os.path.join(args.out, "NODE_SUMMARY.json"),
        {"node": args.node, "jobs": summary, "host": host_facts()},
    )
    print("XS_NODE_DONE", args.node, len(jobs))
    return 0
