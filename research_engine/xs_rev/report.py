...
from research_engine.xs_rev import XS_ID
from research_engine.xs_rev.space import canonical_search_space_hash
...
def decide(ranking):
    ...
    return {
        ...
        "note": "Do not retune lookback/k/hold or flip to momentum. Do not read Final OOS.",
    }
...
def write_outputs(out_dir, ranking, decision, hashes=None):
    if not os.path.isdir(out_dir):
        os.makedirs(out_dir)
    dump_json(os.path.join(out_dir, "XS_RANKING_V1.json"), ranking)
    dump_json(os.path.join(out_dir, "XS_DECISION_V1.json"), decision)
    md = render_markdown(ranking, decision, hashes=hashes)
    path = os.path.join(out_dir, "XS_REPORT_V1.md")
