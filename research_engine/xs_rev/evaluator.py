...
from research_engine.xs_rev import HOLD_BARS, OCCUPANCY_FAIL, XS_BLOCK, XS_BOOT, XS_PERM, XS_SEED
from research_engine.xs_rev.contract import deny_final_oos
from research_engine.xs_rev.event_detector import fired_at
from research_engine.xs_rev.windows import same_role_hold
...
def _legs(aligned, row, mode):
    longs = list(row.get("laggards") or [])
    shorts = list(row.get("leaders") or [])
    if mode == "LAG_LONG":
        shorts = []
    out = []
    for name in longs:
