"""V38-L1 runner: pre-announcement layer, one pre-registered model, LO20 gate book, A4 independence vs ML1 (and V27 ML2F)."""
...
from research_engine.cn_a_share_preann_v38 import EQUITY, FDR_Q, HOLD_DAYS, NORM, OUT, RESEARCH, SAME_CLUSTER_CORR, TRADES, V38L1_ID, VALIDATION, ensure_v38l1
from research_engine.cn_a_share_preann_v38.contract import HYPOTHESES, build_contract
from research_engine.cn_a_share_preann_v38.features import NAMES, PA_FEATURES, build_features
...
def main():
    ...
    if os.path.isfile(os.path.join(OUT, "DECISION.json")):
        raise RuntimeError("V38L1_ALREADY_RUN: one run only; a second configuration is a new contract")
    if not pack_exists():
        pack_panel()
    pack = load_pack()
    dates = pack["dates"]
    ...
    feats = build_features(pack, elig)
    pit = json.load(open(os.path.join(NORM, "PREANN_PIT.json"), encoding="utf-8"))
    contract = build_contract(pit["content_hash"])
    dump_json(os.path.join(OUT, "CONTRACT.json"), contract)
    progress("CONTRACT", contract_hash=contract["contract_hash"], preann_hash=pit["content_hash"])
    cover = {}
    for n in NAMES:
        a = feats[n]
    ...
    progress("MODEL_START", signal="ML4P_LGBM")
    sc, meta = build_scores(pack, feats, elig, xok, features=PA_FEATURES, tag="V38L1_ML4P")
    scores = sc["ML1_LGBM"]
    np.save(os.path.join(OUT, "SCORES_ML4P_LGBM.npy"), scores)
    dump_json(os.path.join(OUT, "MODEL_ML4P_LGBM.json"), meta)
    progress("MODEL_DONE")
