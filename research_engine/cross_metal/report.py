"""CROSS_METAL report and decision. No hash rewrite. No 10% claim."""
from __future__ import print_function

import os

from research_engine.cross_metal import CM_ID
from research_engine.cross_metal.space import canonical_search_space_hash
...
def decide(ranking):
    ...
    return {
        ...
        "note": "Do not retune z_cut or flip sign. Do not read Final OOS. Do not reopen residual.",
    }
...
def render_markdown(ranking, decision, hashes=None):
    ...
    lines = [
        ...
        "- SILVER pass: %s" % ranking.get("silver_pass"),
        "",
        "## Hypotheses",
        "",
    ]
    ...
    if hashes:
        ...
        lines.append("")
    lines.append("CAGR >= 10% is not a gate. Not V0.91 SMA60 residual. Not a z_cut search.")
    lines.append("")
    return "\n".join(lines)
...
def write_outputs(out_dir, ranking, decision, hashes=None):
    if not os.path.isdir(out_dir):
        os.makedirs(out_dir)
    dump_json(os.path.join(out_dir, "CM_RANKING_V1.json"), ranking)
    dump_json(os.path.join(out_dir, "CM_DECISION_V1.json"), decision)
    md = render_markdown(ranking, decision, hashes=hashes)
    path = os.path.join(out_dir, "CM_REPORT_V1.md")
