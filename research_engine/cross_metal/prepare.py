...
from research_engine.cross_metal import PARENTS
from research_engine.cross_metal.contract import assert_search_space
from research_engine.cross_metal.data import load_parents
from research_engine.cross_metal.space import build_search_space
from research_engine.cross_metal.windows import freeze_window, oos_exists
    ...
    for name, row in packed.items():
        ...
        dump_json(
            os.path.join(win_dir, "EVENTS_%s.json" % name),
            {
                ...
                "n_ratio_rich_cross": sum(1 for b in row["bars"] if b.get("is_ratio_rich_cross")),
                "n_ratio_cheap_cross": sum(1 for b in row["bars"] if b.get("is_ratio_cheap_cross")),
            },
        )
    return packed, space
