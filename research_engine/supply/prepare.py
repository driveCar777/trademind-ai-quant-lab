...
from research_engine.supply import PARENTS
from research_engine.supply.contract import assert_search_space
from research_engine.supply.data import load_parents
from research_engine.supply.space import build_search_space
from research_engine.supply.windows import freeze_window, oos_exists
    ...
    for name, row in packed.items():
        ...
        dump_json(
            os.path.join(win_dir, "EVENTS_%s.json" % name),
            {
                ...
                "n_prod_drop_cross": sum(1 for b in row["bars"] if b.get("is_prod_drop_cross")),
                "n_util_up_cross": sum(1 for b in row["bars"] if b.get("is_util_up_cross")),
            },
        )
    return packed, space
