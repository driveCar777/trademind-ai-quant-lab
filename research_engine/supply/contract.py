...
from research_engine.supply.space import canonical_search_space_hash
...
def assert_search_space(space):
    if not space or space.get("discovery_id") != SUP_ID:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if int(space.get("seed") or 0) != SUP_SEED:
        raise ContractMismatch("CONTRACT_MISMATCH")
...
def assert_job_contract(job, space):
    ...
    if job.get("discovery_id") != SUP_ID:
        raise ContractMismatch("CONTRACT_MISMATCH")
...
def deny_final_oos(role=None):
    if role is None:
        final_oos_access(reason="inventory_v1")
