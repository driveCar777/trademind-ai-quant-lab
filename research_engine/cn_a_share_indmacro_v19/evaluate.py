...
from research_engine.cn_a_share_indmacro_v19 import DENIED, FDR_Q, HOLD_DAYS, RESEARCH, SAME_CLUSTER_CORR, VALIDATION
from research_engine.cn_a_share_indmacro_v19.contract import HYPOTHESES
from research_engine.cn_a_share_indmacro_v19.paths import EQUITY, OUT, TRADES
...
def run_one(pack, elig, xok, ew, scores, hyp):
    hid = hyp["id"]
    print("V19", hid, "PRED", flush=True)
    ...
    print("V19", hid, "CAPITAL", flush=True)
...
def apply_gates(recs):
    ...
    for i, r in enumerate(recs):
        ...
        if ok:
            ...
            failures.append({"id": r["id"], "family": r["family"], "mechanism": r["mechanism"], "why_failed": [k for k, v in checks.items() if v is False], "reopen": "New contract only."})
...
def decide(candidates, ran):
    ...
    if independent:
        ...
        overall, nxt, stop = "A_SHARE_INDUSTRY_MACRO_V1_NO_CANDIDATE", "INDUSTRY_MACRO_NO_CANDIDATE", "STOP_B_FAMILY"
    else:
