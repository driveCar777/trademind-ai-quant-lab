#!/usr/bin/env python3
"""Xavier FUTURES_OI_FLOW worker. Reads locked space. Does not invent hypotheses."""
...
from research_engine.curve_eia import DATASET_ID, CUEIA_ID
from research_engine.curve_eia.contract import assert_job_contract
...
from research_engine.curve_eia.space import hypothesis_map
...
def run_job(job, space, market_root, out_dir):
    ...
    for hid in job["hypothesis_ids"]:
        ...
        hyps.append(
            run_hypothesis(
                spec,
                packed,
                seed=int(job.get("seed") or 20260830),
    ...
    payload = {
        ...
        "parent_hashes": {DATASET_ID: packed.get("sha256")},
        "window_hashes": {"BOOK": window.get("window_hash")},
...
def main(argv=None):
    ...
    for job in jobs:
        ...
        print("CUEIA_JOB_DONE", job["job_id"], "%.1f" % payload["elapsed_seconds"])
    dump_json(
        os.path.join(args.out, "NODE_SUMMARY.json"),
        {"node": args.node, "jobs": summary, "host": host_facts()},
    )
    print("CUEIA_NODE_DONE", args.node, len(jobs))
    return 0
