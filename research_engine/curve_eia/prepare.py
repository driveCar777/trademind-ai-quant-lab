...
from research_engine.curve_eia import CURVE_ID, ROOTS
from research_engine.curve_eia.contract import assert_search_space
from research_engine.curve_eia.data import load_panel
from research_engine.curve_eia.space import build_search_space
...
def prepare_pack(market_root, out_dir, space=None):
    ...
    if os.path.exists(path):
        ...
        window = freeze_window(book, "CUEIA_BOOK")
    ...
    dump_json(
        os.path.join(win_dir, "EVENTS_BOOK.json"),
        {
            ...
            "n_inv_build_steepen": sum(
                1
                for b in book
                if any((b.get("roots") or {}).get(r) and (b["roots"][r] or {}).get("is_inv_build_steepen") for r in ROOTS)
            ),
            "n_inv_draw_flatten": sum(
                1
                for b in book
                if any((b.get("roots") or {}).get(r) and (b["roots"][r] or {}).get("is_inv_draw_flatten") for r in ROOTS)
            ),
            "n_inv_draw_steepen": sum(
                1
                for b in book
                if any((b.get("roots") or {}).get(r) and (b["roots"][r] or {}).get("is_inv_draw_steepen") for r in ROOTS)
            ),
        },
    )
    return packed, space
