...
from research_engine.mt5_macro_v32 import DATA, DATASET_ID, EXCLUDE, GROUPS, INDICES, ensure
...
def main():
    ...
    syms = [s for s in mt5.symbols_get() if (s.path.startswith(GROUPS) or s.name in INDICES) and s.name not in EXCLUDE and s.trade_mode != 0]
    specs, n_rows, t0 = [], 0, time.time()
    h = hashlib.sha256()
    for k, s in enumerate(syms):
        mt5.symbol_select(s.name, True)
        r = mt5.copy_rates_from_pos(s.name, mt5.TIMEFRAME_D1, 0, 8000)
    ...
    mt5.shutdown()
    man = {"dataset_id": DATASET_ID, "source": "mt5", "broker": ti.company, "terminal": ti.name, "timeframe": "D1", "universe": "Forex Majors/Crosses + CFD Metals/Energies/Agri/Bonds + named equity indices (current listing only)",
