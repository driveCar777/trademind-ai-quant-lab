...
from research_engine.hot_mt5_gold_h1_v6.engine import run
from research_engine.hot_mt5_gold_h1_v6.paths import RES
...
def main(argv=None):
    ...
    if (RES / "READ.json").is_file() and not args.force:
        print("READ exists — refuse second train of HOT_MT5_GOLD_H1_V6_RIDGE")
        return 0
    summary = run()
    if not summary.get("ok"):
        print(summary)
        return 1
    book = summary["books"]["H1_RIDGE"]
    ...
    print("IN-SAMPLE ic=%s r2=%s hit=%s | fold train_ic=%s test_ic=%s" % (
    ...
    print("H1_RIDGE %s long TWR=%s hit=%s | val n=%s TWR=%s t=%s | month TWR=%s | %s" % (
        book["verdict"],
    ...
    print("SUMMARY viable", summary["n_viable"], "/ 1 candidate=false Ridge alpha=1")
    return 0
