...
from research_engine.curve_rate import CURATE_FDR_Q
...
def rank_program(rows):
    ...
    for row in rows:
        ...
        payload.append(item)
    fdr = benjamini_hochberg(pvals, q=CURATE_FDR_Q)
    i = 0
    passed = []
    fdr_pass = []
    while i < len(payload):
    ...
    return {
        "outcome": outcome,
        "fdr": {"q": CURATE_FDR_Q, "m": fdr.get("m"), "discoveries": fdr.get("discoveries")},
        "book_pass": passed,
        "fdr_pass": fdr_pass,
        "hypotheses": payload,
        "note": "CANDIDATE needs two book hyps plus FDR. Not 10%. Do not retune yield change or GC steepening.",
    }
