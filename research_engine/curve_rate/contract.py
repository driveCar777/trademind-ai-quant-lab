...
from research_engine.curve_rate.space import search_space_hash
from research_engine.holdout import assert_role_allowed, final_oos_access


def assert_search_space(space):
    if not space or space.get("family_id") != CURATE_ID:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if int(space.get("seed") or 0) != CURATE_SEED:
        raise ContractMismatch("CONTRACT_MISMATCH")
...
def assert_job_contract(job, space):
    ...
    if job.get("discovery_id") != CURATE_ID:
        raise ContractMismatch("CONTRACT_MISMATCH")
...
def deny_final_oos(role=None):
    if role is None:
        final_oos_access(reason="curve_realyield_v1")
