#!/usr/bin/env python3
"""Xavier V0.9 worker. Reads locked space. Does not invent hypotheses."""
...
from research_engine.regime_transition import PARENTS, RT_ID
from research_engine.regime_transition.contract import assert_job_contract
from research_engine.regime_transition.hypothesis_runner import run_hypothesis
from research_engine.regime_transition.prepare import prepare_pack
from research_engine.regime_transition.space import hypothesis_map
...
def main(argv=None):
    ...
    for job in jobs:
        ...
        print("RT_JOB_DONE", job["job_id"], "%.1f" % payload["elapsed_seconds"])
    dump_json(os.path.join(args.out, "NODE_SUMMARY.json"), {"node": args.node, "jobs": summary})
    print("RT_NODE_DONE", args.node, len(jobs))
    return 0
