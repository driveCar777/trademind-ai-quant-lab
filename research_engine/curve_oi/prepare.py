"""Freeze space and the curve-OI book window before any costed equity."""
...
from research_engine.curve_oi.data import load_panel
from research_engine.curve_oi.space import build_search_space
...
def prepare_pack(market_root, out_dir, space=None):
    ...
    if os.path.exists(path):
        ...
        window = freeze_window(book, "CUROI_OI_BOOK")
    ...
    dump_json(
        os.path.join(win_dir, "EVENTS_BOOK.json"),
        {
            ...
            "n_steepen_oi_expand": sum(
                1
                for b in book
                if any((b.get("roots") or {}).get(r) and (b["roots"][r] or {}).get("is_steepen_oi_expand") for r in ROOTS)
            ),
            "n_flatten_oi_expand": sum(
                1
                for b in book
                if any((b.get("roots") or {}).get(r) and (b["roots"][r] or {}).get("is_flatten_oi_expand") for r in ROOTS)
            ),
            "n_steepen_oi_contract": sum(
                1
                for b in book
                if any((b.get("roots") or {}).get(r) and (b["roots"][r] or {}).get("is_steepen_oi_contract") for r in ROOTS)
            ),
        },
    )
    return packed, space
