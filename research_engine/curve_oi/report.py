...
from research_engine.curve_oi import LOCKED_HASH, CUROI_ID
...
def decide(ranking):
    ...
    return {
        ...
        "note": "Do not retune steepening/OI/hold. Do not use Ava CFD as spot. Do not read Final OOS.",
    }
...
def write_outputs(out_dir, ranking, decision, hashes=None):
    if not os.path.isdir(out_dir):
        os.makedirs(out_dir)
    dump_json(os.path.join(out_dir, "CUROI_RANKING_V1.json"), ranking)
    dump_json(os.path.join(out_dir, "CUROI_DECISION_V1.json"), decision)
    md = render_markdown(ranking, decision, hashes=hashes)
    path = os.path.join(out_dir, "CUROI_REPORT_V1.md")
