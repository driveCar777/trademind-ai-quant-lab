...
from research_engine.curve_oi.space import search_space_hash
from research_engine.holdout import assert_role_allowed, final_oos_access


def assert_search_space(space):
    if not space or space.get("family_id") != CUROI_ID:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if int(space.get("seed") or 0) != CUROI_SEED:
        raise ContractMismatch("CONTRACT_MISMATCH")
...
def assert_job_contract(job, space):
    ...
    if job.get("discovery_id") != CUROI_ID:
        raise ContractMismatch("CONTRACT_MISMATCH")
...
def deny_final_oos(role=None):
    if role is None:
        final_oos_access(reason="curve_oi_joint_v1")
