...
from research_engine.microstructure_surprise import MS_ID
from research_engine.microstructure_surprise.contract import assert_job_contract
...
from research_engine.microstructure_surprise.space import hypothesis_map
...
def main(argv=None):
    ...
    for job in bundle.get("jobs") or []:
        ...
        print("MS_JOB_DONE", job["job_id"], "%.1f" % payload["elapsed_seconds"])
    dump_json(os.path.join(args.out, "NODE_SUMMARY.json"), {"node": args.node, "jobs": summary, "host": host_facts()})
    print("MS_NODE_DONE", args.node, len(summary))
    return 0
