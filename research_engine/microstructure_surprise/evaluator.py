"""Hold=5 NEXT_BAR_OPEN surprise book. Overlap skip. No close fill. No Final OOS."""
from __future__ import print_function

from research_engine.microstructure_surprise import HOLD_BARS, MS_BLOCK, MS_BOOT, MS_PERM, MS_SEED, OCCUPANCY_FAIL
from research_engine.microstructure_surprise.contract import deny_final_oos
from research_engine.microstructure_surprise.event_detector import fired_at
from research_engine.microstructure_surprise.windows import same_role_hold
    ...
    for role in ("research", "validation"):
        ...
        while i < len(bars):
            ...
            if role_i == role:
                ...
                if fired:
                    n_signal += 1
                if signal_t_ok(bars, i, role, hold_bars):
                    n_eligible += 1
                ...
                curve.append(cash)
            i += 1
        metrics = summarize_equity(curve if curve else [START_EQUITY], "H1", START_EQUITY, trades)
...
def signal_t_ok(bars, signal_t, role, hold_bars):
    if signal_t < 0:
        return False
