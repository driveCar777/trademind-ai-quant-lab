...
from research_engine.microstructure_surprise import PARENTS
from research_engine.microstructure_surprise.contract import assert_search_space
from research_engine.microstructure_surprise.data import load_parents
from research_engine.microstructure_surprise.space import build_search_space
from research_engine.microstructure_surprise.windows import freeze_window, oos_exists
    ...
    for name, row in packed.items():
        ...
        dump_json(
            os.path.join(win_dir, "EVENTS_%s.json" % name),
            {
                ...
                "n_surprise": sum(1 for b in row["bars"] if b.get("is_vol_surprise")),
                "n_div": sum(1 for b in row["bars"] if b.get("is_vol_div")),
            },
        )
    return packed, space
