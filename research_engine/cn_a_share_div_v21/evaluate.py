"""V21 dual-book evaluation. MEAN_FORWARD_RETURN is not CAGR."""
...
from research_engine.cn_a_share_alpha_v2.books import ic_series
...
from research_engine.cn_a_share_div_v21 import DENIED, FDR_Q, HOLD_DAYS, RESEARCH, SAME_CLUSTER_CORR, VALIDATION
from research_engine.cn_a_share_div_v21.paths import EQUITY, OUT, TRADES
from research_engine.cn_a_share_index_v20.books import capital_book_set, overlapping_predictive_set
    ...
    return _json_safe(
        {
            ...
            "n_pred_days": r.get("n_pred_days"),
            "n_cap_trades": r.get("n_cap_trades"),
        }
    )
...
def run_one(pack, elig, xok, ew, cache, hyp):
    ...
    min_n = int(hyp["min_n"])
    print("V21", hid, "PRED", flush=True)
    pred = overlapping_predictive_set(pack, scores, elig, xok, RESEARCH[0], VALIDATION[1], HOLD_DAYS, min_n=min_n)
    ...
    print("V21", hid, "CAPITAL", flush=True)
    cap_r = capital_book_set(pack, scores, elig, xok, RESEARCH[0], RESEARCH[1], HOLD_DAYS, min_n=min_n)
    cap_v = capital_book_set(pack, scores, elig, xok, VALIDATION[0], VALIDATION[1], HOLD_DAYS, min_n=min_n)
    cap_f = capital_book_set(pack, scores, elig, xok, RESEARCH[0], VALIDATION[1], HOLD_DAYS, min_n=min_n)
    rec = {
        "id": hid,
        "family": hyp["family"],
        "signal": hyp["signal"],
        "mechanism": hyp["mechanism"],
        "n_pred_days": len(pred),
        "n_cap_trades": len(cap_f.get("trades") or []),
        "predictive": {
            "research": dict(summarize_predictive(pred_r), excess_vs_b0=_finite(ex_r), excess_t=_finite(et_r), excess_p=_finite(ep_r), rank_ic=_finite(float(np.mean(ic_r)) if ic_r else None)),
            "validation": dict(summarize_predictive(pred_v), excess_vs_b0=_finite(ex_v), excess_t=_finite(et_v), excess_p=_finite(ep_v), rank_ic=_finite(float(np.mean(ic_v)) if ic_v else None)),
        },
        "capital": {
    ...
    write_csv(os.path.join(EQUITY, "%s.csv" % hid), ("signal_date", "exit", "capital_ret", "equity"), [{"signal_date": tr["signal_date"], "exit": tr["exit"], "capital_ret": tr["capital_ret"], "equity": tr["equity"]} for tr in cap_f["trades"]])
    write_csv(os.path.join(TRADES, "%s.csv" % hid), ("signal_date", "entry", "exit", "n_sel", "n_fill", "capital_ret", "net_yuan"), cap_f["trades"])
    return rec


def apply_gates(recs):
    ...
    for i, r in enumerate(recs):
        ...
        for hid in ("H11_VOL_60", "H12_VOL_120"):
            cluster[hid] = {"capital": corr_maps(r.get("period_rets") or {}, legacy[hid]["capital"]), "predictive": corr_maps(r.get("pred_rets") or {}, legacy[hid]["predictive"])}
        ...
        if ok:
            ...
            failures.append({"id": r["id"], "family": r["family"], "mechanism": r["mechanism"], "why_failed": [k for k, v in checks.items() if v is False], "reopen": "New contract only. Do not retune. Do not flip sign."})
    ...
    dump_json(os.path.join(OUT, "CANDIDATES.json"), {"n": len(candidates), "ids": [c["id"] for c in candidates], "independent": [c["id"] for c in candidates if c.get("cluster_tag") == "POTENTIALLY_INDEPENDENT"], "same_cluster": [c["id"] for c in candidates if c.get("cluster_tag") == "SAME_CLUSTER"], "tags": dict((c["id"], c.get("cluster_tag")) for c in candidates)})
...
def decide(candidates, ran):
    ...
    if independent:
        ...
        overall, nxt, stop = "A_SHARE_DIVIDEND_EVENT_V1_NO_CANDIDATE", "DIVIDEND_EVENT_NO_CANDIDATE", "STOP_B_FAMILY"
    else:
        overall, nxt, stop = "READY_BUT_NOT_RUN", "RUN_ALPHA", "INCOMPLETE"
    return {
        "LEVEL": 1,
