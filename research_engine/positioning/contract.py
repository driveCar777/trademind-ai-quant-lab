...
from research_engine.positioning.space import canonical_search_space_hash
...
def assert_search_space(space):
    if not space or space.get("discovery_id") != POS_ID:
        raise ContractMismatch("CONTRACT_MISMATCH")
    if int(space.get("seed") or 0) != POS_SEED:
        raise ContractMismatch("CONTRACT_MISMATCH")
...
def assert_job_contract(job, space):
    ...
    if job.get("discovery_id") != POS_ID:
        raise ContractMismatch("CONTRACT_MISMATCH")
...
def deny_final_oos(role=None):
    if role is None:
        final_oos_access(reason="positioning_v1")
