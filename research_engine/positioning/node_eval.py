#!/usr/bin/env python3
"""Xavier IV worker. Reads locked space. Does not invent hypotheses."""
...
from research_engine.positioning import POS_ID
from research_engine.positioning.contract import assert_job_contract
...
from research_engine.positioning.space import hypothesis_map
...
def _targets(packed):
    return [k for k in packed if not str(k).startswith("_")]
...
def main(argv=None):
    ...
    for job in jobs:
        ...
        print("POS_JOB_DONE", job["job_id"], "%.1f" % payload["elapsed_seconds"])
    dump_json(
        os.path.join(args.out, "NODE_SUMMARY.json"),
        {"node": args.node, "jobs": summary, "host": host_facts()},
    )
    print("POS_NODE_DONE", args.node, len(jobs))
    return 0
