...
from research_engine.positioning import PARENTS
from research_engine.positioning.contract import assert_search_space
from research_engine.positioning.data import load_parents
from research_engine.positioning.space import build_search_space
from research_engine.positioning.windows import freeze_window, oos_exists
    ...
    for name, row in packed.items():
        ...
        dump_json(
            os.path.join(win_dir, "EVENTS_%s.json" % name),
            {
                ...
                "n_mm_wash_cross": sum(1 for b in row["bars"] if b.get("is_mm_wash_cross")),
                "n_com_long_cross": sum(1 for b in row["bars"] if b.get("is_com_long_cross")),
            },
        )
    return packed, space
