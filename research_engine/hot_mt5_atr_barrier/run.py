...
from research_engine.hot_mt5_atr_barrier.engine import run_all
from research_engine.hot_mt5_atr_barrier.paths import RES
...
def main(argv=None):
    ...
    if (RES / "READ.json").is_file() and not args.force:
        print("READ exists — refuse second train of HOT_MT5_ATR_BARRIER_V3")
        return 0
    summary = run_all(ids)
    for it in summary["items"]:
        ls = it.get("long_sample") or {}
        vs = it.get("validation_30") or {}
        lab = it.get("label_share") or {}
        print("%s %s cash=%.1f%% hurdle=%.2fbp long TWR=%s cov=%s payoff=%s | val n=%s TWR=%s t=%s | %s" % (
            it.get("id"), it.get("verdict"),
            100 * (lab.get("cash") or 0),
            10000 * (it.get("median_hurdle") or 0),
    ...
    print("SUMMARY viable", summary["n_viable"], "/", summary["n_ok"], "candidate=false atr_k=1")
