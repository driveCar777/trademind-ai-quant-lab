#!/usr/bin/env python3
"""IDX_ASYNC V1 dispatcher. Env password only. Does not invent IDs."""
...
from research_engine.idx_async import LOCKED_HASH, PARENTS, IA_ID
from research_engine.idx_async.contract import assert_job_contract, assert_search_space
from research_engine.idx_async.hypothesis_runner import run_hypothesis
from research_engine.idx_async.jobs import build_all_jobs
from research_engine.idx_async.prepare import prepare_pack
from research_engine.idx_async.rank import rank_program
from research_engine.idx_async.report import decide, write_outputs
from research_engine.idx_async.space import build_search_space, hypothesis_map
...
OUT = os.path.join(ROOT, "data", "market", "research_engine", "idx_async")
...
def lock_space():
    path = os.path.join(OUT, "IA_SEARCH_SPACE_V1.json")
    if os.path.exists(path):
        ...
        print("IA_SPACE_REUSE", space["search_space_hash"])
        return space
    space = build_search_space()
    assert_search_space(space)
    write_once(path, space)
    print("IA_SPACE_LOCK", space["search_space_hash"])
    return space
...
def local_run(space, packed, iters_boot, iters_perm, tag):
    ...
    ranking["mode"] = tag
    dump_json(os.path.join(OUT, "IA_SMOKE_RANKING.json" if tag == "smoke" else "IA_LOCAL_RANKING.json"), ranking)
    decision = decide(ranking)
    write_outputs(OUT, ranking, decision, hashes={"search_space_hash": space["search_space_hash"]})
    print("IA_LOCAL_OUTCOME", ranking["outcome"], decision["next_action"])
...
def run_node(node, jobs, space):
    ...
    bundle = {"discovery_id": IA_ID, "node": node["name"], "jobs": jobs}
    dispatch_path = os.path.join(OUT, "jobs", "dispatch_%s.json" % node["name"])
    dump_json(dispatch_path, bundle)
    space_path = os.path.join(OUT, "IA_SEARCH_SPACE_V1.json")
    for attempt in (1, 2):
        client = None
        try:
            client = connect(node["host"])
            ...
            sftp.put(space_path, REMOTE + "/contracts/IA_SEARCH_SPACE_V1.json")
            ...
            sftp.close()
            cmd = (
                "cd %s && PYTHONPATH=%s python3 research_engine/idx_async/node_eval.py "
                "--node %s --jobs-dir %s/jobs --out %s/out "
                "--contracts-file %s/NODE_JOBS.json "
                "--space-file %s/contracts/IA_SEARCH_SPACE_V1.json"
...
def main(argv=None):
    ...
    if args.mode in ("full", "xavier"):
        ...
        if [p for p in pre if not p.get("ok")]:
            print("IA_PREFLIGHT_FAIL using local-full as authority")
            return 0
        ...
        if primary:
            ...
            dump_json(os.path.join(OUT, "IA_RANKING_V1.json"), ranking)
            print("IA_OUTCOME", ranking["outcome"], "cross_check", ranking.get("cross_check_match"))
    elif ranking is not None:
        print("IA_OUTCOME_LOCAL", ranking.get("outcome"))
    return 0
