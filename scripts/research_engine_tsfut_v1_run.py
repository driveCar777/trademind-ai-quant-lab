#!/usr/bin/env python3
"""TERM_STRUCTURE V1 dispatcher. Env password only. Never ships the Databento key."""
...
from research_engine.term_structure import DATASET_ID, LOCKED_HASH, TS_ID
from research_engine.term_structure.contract import assert_job_contract, assert_search_space
from research_engine.term_structure.hypothesis_runner import run_hypothesis
from research_engine.term_structure.jobs import build_all_jobs
from research_engine.term_structure.prepare import prepare_pack
from research_engine.term_structure.rank import rank_program
from research_engine.term_structure.report import decide, write_outputs
from research_engine.term_structure.space import build_search_space, hypothesis_map
from xavier_auth import xavier_password, xavier_user


USER = xavier_user()
REMOTE = "/tmp/tm-tsfut-v1"
MARKET = os.path.join(ROOT, "data", "market", "immutable")
OUT = os.path.join(ROOT, "data", "market", "research_engine", "term_structure")
...
def local_run(space, packed, iters_boot, iters_perm, tag):
    ...
    ranking["mode"] = tag
    dump_json(
        os.path.join(OUT, "TS_SMOKE_RANKING.json" if tag == "smoke" else "TS_LOCAL_RANKING.json"),
        ranking,
    )
...
def run_node(node, jobs, space):
    ...
    for attempt in (1, 2):
        ...
        try:
            ...
            sftp.close()
            cmd = (
                "cd %s && PYTHONPATH=%s python3 research_engine/term_structure/node_eval.py "
...
def main(argv=None):
    ...
    if args.mode == "lock":
        print("TS_LOCK_ONLY", len(space.get("hypothesis_ids") or []))
        return 0
    if args.mode == "smoke":
