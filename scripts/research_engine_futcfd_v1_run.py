#!/usr/bin/env python3
"""FUT_CFD_LEAD V1 dispatcher. Env password only. Never ships the Databento key."""
...
from research_engine.fut_cfd import CURVE_ID, GOLD_ID, LOCKED_HASH, OIL_ID, FUTCFD_ID
from research_engine.fut_cfd.contract import assert_job_contract, assert_search_space
from research_engine.fut_cfd.hypothesis_runner import run_hypothesis
from research_engine.fut_cfd.jobs import build_all_jobs
from research_engine.fut_cfd.prepare import prepare_pack
from research_engine.fut_cfd.rank import rank_program
from research_engine.fut_cfd.report import decide, write_outputs
from research_engine.fut_cfd.space import build_search_space, hypothesis_map
from research_engine.io_util import dump_json, load_json, write_once
from xavier_auth import xavier_password, xavier_user


USER = xavier_user()
REMOTE = "/tmp/tm-futcfd-v1"
MARKET = os.path.join(ROOT, "data", "market", "immutable")
OUT = os.path.join(ROOT, "data", "market", "research_engine", "fut_cfd")
...
def lock_space():
    path = os.path.join(OUT, "FUTCFD_SEARCH_SPACE_V1.json")
    if os.path.exists(path):
        ...
        print("FUTCFD_SPACE_REUSE", space["search_space_hash"])
        return space
    space = build_search_space()
    assert_search_space(space)
    write_once(path, space)
    print("FUTCFD_SPACE_LOCK", space["search_space_hash"])
    return space
...
def local_run(space, packed, iters_boot, iters_perm, tag):
    print("FUTCFD_LOCAL_START", tag, now())
    ...
    ranking["mode"] = tag
    dump_json(
        os.path.join(OUT, "FUTCFD_SMOKE_RANKING.json" if tag == "smoke" else "FUTCFD_LOCAL_RANKING.json"),
        ranking,
    )
    decision = decide(ranking)
    write_outputs(OUT, ranking, decision, hashes={"search_space_hash": space["search_space_hash"]})
    print("FUTCFD_LOCAL_OUTCOME", ranking["outcome"], decision["next_action"])
...
def run_node(node, jobs, space):
    ...
    bundle = {"discovery_id": FUTCFD_ID, "node": node["name"], "jobs": jobs}
    dispatch_path = os.path.join(OUT, "jobs", "dispatch_%s.json" % node["name"])
    dump_json(dispatch_path, bundle)
    space_path = os.path.join(OUT, "FUTCFD_SEARCH_SPACE_V1.json")
    for attempt in (1, 2):
        client = None
        try:
            client = connect(node["host"])
            ...
            sftp.put(space_path, REMOTE + "/contracts/FUTCFD_SEARCH_SPACE_V1.json")
            ship = (
                (CURVE_ID, ("curve.csv", "manifest.json", "DATA_QUALITY.json")),
                (GOLD_ID, ("bars.csv", "manifest.json")),
                (OIL_ID, ("bars.csv", "manifest.json")),
            )
            for ds, files in ship:
                remote_ds = "%s/jobs/%s" % (REMOTE, ds)
                ssh_run(client, "mkdir -p %s" % remote_ds, 20)
                local = os.path.join(MARKET, ds)
                for fname in files:
            ...
            sftp.close()
            cmd = (
                "cd %s && PYTHONPATH=%s python3 research_engine/fut_cfd/node_eval.py "
                "--node %s --jobs-dir %s/jobs --out %s/out "
                "--contracts-file %s/NODE_JOBS.json "
                "--space-file %s/contracts/FUTCFD_SEARCH_SPACE_V1.json"
...
def main(argv=None):
    ...
    ensure_dirs()
    print("FUTCFD_START", now(), args.mode)
    space = lock_space()
    packed, space = prepare_pack(MARKET, OUT, space=space)
    jobs_payload = lock_jobs(space)
    if args.mode == "lock":
        print("FUTCFD_LOCK_ONLY", len(space.get("hypothesis_ids") or []))
        return 0
    if args.mode == "smoke":
    ...
    if args.mode in ("full", "xavier"):
        ...
        if [p for p in pre if not p.get("ok")]:
            print("FUTCFD_PREFLIGHT_FAIL using local-full as authority")
            return 0
        ...
        if primary:
            ...
            print("FUTCFD_OUTCOME", ranking["outcome"], "cross_check", ranking.get("cross_check_match"))
    elif ranking is not None:
        print("FUTCFD_OUTCOME_LOCAL", ranking.get("outcome"))
    return 0
