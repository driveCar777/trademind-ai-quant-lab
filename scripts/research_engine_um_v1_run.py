...
from research_engine.usd_metal import LOCKED_HASH, PARENTS, UM_ID
from research_engine.usd_metal.contract import assert_job_contract, assert_search_space
from research_engine.usd_metal.hypothesis_runner import run_hypothesis
from research_engine.usd_metal.jobs import build_all_jobs
from research_engine.usd_metal.prepare import prepare_pack
from research_engine.usd_metal.rank import rank_program
from research_engine.usd_metal.report import decide, write_outputs
from research_engine.usd_metal.space import build_search_space, hypothesis_map
...
OUT = os.path.join(ROOT, "data", "market", "research_engine", "usd_metal")
...
def lock_space():
    path = os.path.join(OUT, "UM_SEARCH_SPACE_V1.json")
    if os.path.exists(path):
        ...
        print("UM_SPACE_REUSE", space["search_space_hash"])
        return space
    space = build_search_space()
    assert_search_space(space)
    write_once(path, space)
    print("UM_SPACE_LOCK", space["search_space_hash"])
    return space
...
def local_run(space, packed, iters_boot, iters_perm, tag):
    ...
    ranking["mode"] = tag
    dump_json(os.path.join(OUT, "UM_SMOKE_RANKING.json" if tag == "smoke" else "UM_LOCAL_RANKING.json"), ranking)
    decision = decide(ranking)
    write_outputs(OUT, ranking, decision, hashes={"search_space_hash": space["search_space_hash"]})
    print("UM_LOCAL_OUTCOME", ranking["outcome"], decision["next_action"])
...
def run_node(node, jobs, space):
    ...
    bundle = {"discovery_id": UM_ID, "node": node["name"], "jobs": jobs}
    dispatch_path = os.path.join(OUT, "jobs", "dispatch_%s.json" % node["name"])
    dump_json(dispatch_path, bundle)
    space_path = os.path.join(OUT, "UM_SEARCH_SPACE_V1.json")
    for attempt in (1, 2):
        client = None
        try:
            client = connect(node["host"])
            ...
            sftp.put(space_path, REMOTE + "/contracts/UM_SEARCH_SPACE_V1.json")
            ...
            sftp.close()
            cmd = (
                "cd %s && PYTHONPATH=%s python3 research_engine/usd_metal/node_eval.py "
                "--node %s --jobs-dir %s/jobs --out %s/out "
                "--contracts-file %s/NODE_JOBS.json "
                "--space-file %s/contracts/UM_SEARCH_SPACE_V1.json"
...
def main(argv=None):
    ...
    if args.mode in ("full", "xavier"):
        ...
        if [p for p in pre if not p.get("ok")]:
            print("UM_PREFLIGHT_FAIL using local-full as authority")
            return 0
        ...
        if primary:
            ...
            dump_json(os.path.join(OUT, "UM_RANKING_V1.json"), ranking)
            print("UM_OUTCOME", ranking["outcome"], "cross_check", ranking.get("cross_check_match"))
    elif ranking is not None:
        print("UM_OUTCOME_LOCAL", ranking.get("outcome"))
    return 0
