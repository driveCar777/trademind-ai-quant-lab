#!/usr/bin/env python3
"""Regime Interaction V1 dispatcher. Reuses SSH helpers. Does not invent IDs."""
...
from research_engine.regime_interaction import LOCKED_HASH, PARENTS, RI_ID
from research_engine.regime_interaction.contract import assert_job_contract, assert_search_space
from research_engine.regime_interaction.hypothesis_runner import run_hypothesis
from research_engine.regime_interaction.jobs import build_all_jobs
from research_engine.regime_interaction.prepare import prepare_pack
from research_engine.regime_interaction.rank import rank_program
from research_engine.regime_interaction.report import decide, write_outputs
from research_engine.regime_interaction.space import build_search_space, hypothesis_map
...
OUT = os.path.join(ROOT, "data", "market", "research_engine", "regime_interaction")
REMOTE = "/tmp/tm-ri-v1"
...
def lock_space():
    path = os.path.join(OUT, "RI_SEARCH_SPACE_V1.json")
    if os.path.exists(path):
        ...
        print("RI_SPACE_REUSE", space["search_space_hash"])
        return space
    space = build_search_space()
    assert_search_space(space)
    write_once(path, space)
    print("RI_SPACE_LOCK", space["search_space_hash"])
    return space
...
def local_run(space, packed, iters_boot, iters_perm, tag):
    ...
    for hid in space["hypothesis_ids"]:
        ...
        rows.append(out)
        print("RI_LOCAL", hid, "trades_r", out["research"]["n_trade"], "occ", out["research"]["occupancy"], "p", out["research"]["raw_p"])
    ranking = rank_program(rows)
    ranking["search_space_hash"] = space["search_space_hash"]
    ranking["mode"] = tag
    dump_json(os.path.join(OUT, "RI_SMOKE_RANKING.json" if tag == "smoke" else "RI_LOCAL_RANKING.json"), ranking)
    decision = decide(ranking)
    write_outputs(OUT, ranking, decision, hashes={"search_space_hash": space["search_space_hash"]})
    print("RI_LOCAL_OUTCOME", ranking["outcome"], decision["next_action"])
    return ranking, decision


def run_node(node, jobs, space):
    record = {"node": node["name"], "host": node["host"], "ok": False, "experiment": RI_ID, "contract_hash": space.get("search_space_hash"), "start": now()}
    started = time.time()
    for job in jobs:
        assert_job_contract(job, space)
    dispatch_path = os.path.join(OUT, "jobs", "dispatch_%s.json" % node["name"])
    dump_json(dispatch_path, {"discovery_id": RI_ID, "node": node["name"], "jobs": jobs})
    space_path = os.path.join(OUT, "RI_SEARCH_SPACE_V1.json")
    client = None
    try:
        ...
        sftp.put(space_path, REMOTE + "/contracts/RI_SEARCH_SPACE_V1.json")
        ...
        sftp.close()
        cmd = (
            "cd %s && PYTHONPATH=%s python3 research_engine/regime_interaction/node_eval.py "
            "--node %s --jobs-dir %s/jobs --out %s/out "
            "--contracts-file %s/NODE_JOBS.json --space-file %s/contracts/RI_SEARCH_SPACE_V1.json"
...
def main(argv=None):
    ...
    if args.mode in ("full", "xavier"):
        ...
        if [p for p in pre if not p.get("ok")]:
            print("RI_PREFLIGHT_FAIL local-full remains authority")
            return 0
        ...
        if [r for r in records if not r.get("ok")]:
            print("RI_XAVIER_FAIL local-full remains authority")
            return 0
        ...
        if primary:
            ...
            ranking["cross_check_match"] = hashes.get("RI-V1-Xavier-01-HYP-RI-0001") == hashes.get("RI-V1-Xavier-04-HYP-RI-0001")
            decision = decide(ranking)
            write_outputs(OUT, ranking, decision, hashes=hashes)
            print("RI_OUTCOME", ranking["outcome"], "cross_check", ranking.get("cross_check_match"))
    return 0
