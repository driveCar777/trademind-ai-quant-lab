...
from research_engine.oi_cot import CURVE_ID, GOLD_COT_ID, LOCKED_HASH, OIL_COT_ID, OICOT_ID
from research_engine.oi_cot.contract import assert_job_contract, assert_search_space
from research_engine.oi_cot.hypothesis_runner import run_hypothesis
from research_engine.oi_cot.jobs import build_all_jobs
from research_engine.oi_cot.prepare import prepare_pack
from research_engine.oi_cot.rank import rank_program
from research_engine.oi_cot.report import decide, write_outputs
from research_engine.oi_cot.space import build_search_space, hypothesis_map
from research_engine.io_util import dump_json, load_json, write_once
from xavier_auth import xavier_password, xavier_user


USER = xavier_user()
REMOTE = "/tmp/tm-oicot-v1"
MARKET = os.path.join(ROOT, "data", "market", "immutable")
OUT = os.path.join(ROOT, "data", "market", "research_engine", "oi_cot")
...
def lock_space():
    path = os.path.join(OUT, "OICOT_SEARCH_SPACE_V1.json")
    if os.path.exists(path):
        ...
        print("OICOT_SPACE_REUSE", space["search_space_hash"])
        return space
    space = build_search_space()
    assert_search_space(space)
    write_once(path, space)
    print("OICOT_SPACE_LOCK", space["search_space_hash"])
    return space
...
def lock_jobs(space):
    ...
    if os.path.exists(path):
        ...
        if payload.get("search_space_hash") != space["search_space_hash"]:
            raise ContractMismatch("CONTRACT_MISMATCH")
        print("OICOT_JOBS_REUSE")
        return payload
    by_node = build_all_jobs(space)
    payload = {
        "discovery_id": OICOT_ID,
...
def local_run(space, packed, iters_boot, iters_perm, tag):
    print("OICOT_LOCAL_START", tag, now())
    ...
    ranking["mode"] = tag
    dump_json(
        os.path.join(OUT, "OICOT_SMOKE_RANKING.json" if tag == "smoke" else "OICOT_LOCAL_RANKING.json"),
        ranking,
    )
    decision = decide(ranking)
    write_outputs(OUT, ranking, decision, hashes={"search_space_hash": space["search_space_hash"]})
    print("OICOT_LOCAL_OUTCOME", ranking["outcome"], decision["next_action"])
...
def run_node(node, jobs, space):
    ...
    bundle = {"discovery_id": OICOT_ID, "node": node["name"], "jobs": jobs}
    dispatch_path = os.path.join(OUT, "jobs", "dispatch_%s.json" % node["name"])
    dump_json(dispatch_path, bundle)
    space_path = os.path.join(OUT, "OICOT_SEARCH_SPACE_V1.json")
    for attempt in (1, 2):
        client = None
        try:
            client = connect(node["host"])
            ...
            sftp.put(space_path, REMOTE + "/contracts/OICOT_SEARCH_SPACE_V1.json")
            ship = (
                (CURVE_ID, ("curve.csv", "manifest.json", "DATA_QUALITY.json")),
                (GOLD_COT_ID, ("series.csv", "manifest.json")),
                (OIL_COT_ID, ("series.csv", "manifest.json")),
            )
            ...
            sftp.close()
            cmd = (
                "cd %s && PYTHONPATH=%s python3 research_engine/oi_cot/node_eval.py "
                "--node %s --jobs-dir %s/jobs --out %s/out "
                "--contracts-file %s/NODE_JOBS.json "
                "--space-file %s/contracts/OICOT_SEARCH_SPACE_V1.json"
...
def main(argv=None):
    ...
    ensure_dirs()
    print("OICOT_START", now(), args.mode)
    space = lock_space()
    packed, space = prepare_pack(MARKET, OUT, space=space)
    jobs_payload = lock_jobs(space)
    if args.mode == "lock":
        print("OICOT_LOCK_ONLY", len(space.get("hypothesis_ids") or []))
        return 0
    if args.mode == "smoke":
    ...
    if args.mode in ("full", "xavier"):
        ...
        if [p for p in pre if not p.get("ok")]:
            print("OICOT_PREFLIGHT_FAIL using local-full as authority")
            return 0
        ...
        if primary:
            ...
            print("OICOT_OUTCOME", ranking["outcome"], "cross_check", ranking.get("cross_check_match"))
    elif ranking is not None:
        print("OICOT_OUTCOME_LOCAL", ranking.get("outcome"))
    return 0
