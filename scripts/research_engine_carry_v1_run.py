#!/usr/bin/env python3
"""Carry V1 dispatcher. Env password only. Does not invent IDs."""
...
from research_engine.carry import FEATURE_PARENTS, LOCKED_HASH, PARENTS, CARRY_ID
from research_engine.carry.contract import assert_job_contract, assert_search_space
from research_engine.carry.hypothesis_runner import run_hypothesis
from research_engine.carry.jobs import build_all_jobs
from research_engine.carry.prepare import prepare_pack
from research_engine.carry.rank import rank_program
from research_engine.carry.report import decide, write_outputs
from research_engine.carry.space import build_search_space, hypothesis_map

USER = xavier_user()
REMOTE = "/tmp/tm-carry-v1a"
MARKET = os.path.join(ROOT, "data", "market", "immutable")
OUT = os.path.join(ROOT, "data", "market", "research_engine", "carry_v1a")
...
def lock_space():
    path = os.path.join(OUT, "CARRY_SEARCH_SPACE_V1A.json")
    if os.path.exists(path):
        ...
        print("CARRY_SPACE_REUSE", space["search_space_hash"])
        return space
    space = build_search_space()
    assert_search_space(space)
    write_once(path, space)
    print("CARRY_SPACE_LOCK", space["search_space_hash"])
    return space
...
def local_run(space, packed, iters_boot, iters_perm, tag):
    print("CARRY_LOCAL_START", tag, now())
    ...
    for hid in space["hypothesis_ids"]:
        ...
        out = run_hypothesis(
            hmap[hid],
            packed,
            seed=int(space.get("seed") or 20260828),
        ...
        rows.append(out)
        print("CARRY_LOCAL", hid, "trades_r", out["research"]["n_trade"], "occ", out["research"]["occupancy"], "p", out["research"]["raw_p"])
    ranking = rank_program(rows)
    ranking["search_space_hash"] = space["search_space_hash"]
    ranking["mode"] = tag
    dump_json(os.path.join(OUT, "CARRY_SMOKE_RANKING.json" if tag == "smoke" else "CARRY_LOCAL_RANKING.json"), ranking)
    decision = decide(ranking)
    write_outputs(OUT, ranking, decision, hashes={"search_space_hash": space["search_space_hash"]})
    print("CARRY_LOCAL_OUTCOME", ranking["outcome"], decision["next_action"])
...
def run_node(node, jobs, space):
    ...
    bundle = {"discovery_id": CARRY_ID, "node": node["name"], "jobs": jobs}
    dispatch_path = os.path.join(OUT, "jobs", "dispatch_%s.json" % node["name"])
    dump_json(dispatch_path, bundle)
    space_path = os.path.join(OUT, "CARRY_SEARCH_SPACE_V1A.json")
    for attempt in (1, 2):
        client = None
        try:
            client = connect(node["host"])
            ...
            sftp.put(space_path, REMOTE + "/contracts/CARRY_SEARCH_SPACE_V1A.json")
            for dataset_id in list(PARENTS) + list(FEATURE_PARENTS):
                remote_ds = "%s/jobs/%s" % (REMOTE, dataset_id)
                ssh_run(client, "mkdir -p %s" % remote_ds, 20)
                local = os.path.join(MARKET, dataset_id)
                for fname in ("bars.csv", "manifest.json", "DATA_QUALITY.json", "series.csv"):
                    src = os.path.join(local, fname)
                    if os.path.isfile(src):
                        sftp.put(src, remote_ds + "/" + fname)
            sftp.close()
            cmd = (
                "cd %s && PYTHONPATH=%s python3 research_engine/carry/node_eval.py "
                "--node %s --jobs-dir %s/jobs --out %s/out "
                "--contracts-file %s/NODE_JOBS.json "
                "--space-file %s/contracts/CARRY_SEARCH_SPACE_V1A.json"
...
def main(argv=None):
    ...
    if args.mode in ("full", "xavier"):
        ...
        if [p for p in pre if not p.get("ok")]:
            print("CARRY_PREFLIGHT_FAIL using local-full as authority")
            return 0
        ...
        if primary:
            ...
            dump_json(os.path.join(OUT, "CARRY_RANKING_V1A.json"), ranking)
            print("CARRY_OUTCOME", ranking["outcome"], "cross_check", ranking.get("cross_check_match"))
    elif ranking is not None:
        print("CARRY_OUTCOME_LOCAL", ranking.get("outcome"))
    return 0
