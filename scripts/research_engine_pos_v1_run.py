#!/usr/bin/env python3
"""Positioning V1 dispatcher. Env password only. Does not invent IDs."""
...
from research_engine.positioning import FEATURE_PARENTS, LOCKED_HASH, PARENTS, POS_ID
from research_engine.positioning.contract import assert_job_contract, assert_search_space
from research_engine.positioning.hypothesis_runner import run_hypothesis
from research_engine.positioning.jobs import build_all_jobs
from research_engine.positioning.prepare import prepare_pack
from research_engine.positioning.rank import rank_program
from research_engine.positioning.report import decide, write_outputs
from research_engine.positioning.space import build_search_space, hypothesis_map

USER = xavier_user()
REMOTE = "/tmp/tm-pos-v1"
MARKET = os.path.join(ROOT, "data", "market", "immutable")
OUT = os.path.join(ROOT, "data", "market", "research_engine", "positioning")
...
def lock_space():
    path = os.path.join(OUT, "POS_SEARCH_SPACE_V1.json")
    if os.path.exists(path):
        ...
        print("POS_SPACE_REUSE", space["search_space_hash"])
        return space
    space = build_search_space()
    assert_search_space(space)
    write_once(path, space)
    print("POS_SPACE_LOCK", space["search_space_hash"])
    return space
...
def local_run(space, packed, iters_boot, iters_perm, tag):
    ...
    for hid in space["hypothesis_ids"]:
        ...
        rows.append(out)
        print("POS_LOCAL", hid, "trades_r", out["research"]["n_trade"], "occ", out["research"]["occupancy"], "p", out["research"]["raw_p"])
    ranking = rank_program(rows)
    ranking["search_space_hash"] = space["search_space_hash"]
    ranking["mode"] = tag
    dump_json(os.path.join(OUT, "POS_SMOKE_RANKING.json" if tag == "smoke" else "POS_LOCAL_RANKING.json"), ranking)
    decision = decide(ranking)
    write_outputs(OUT, ranking, decision, hashes={"search_space_hash": space["search_space_hash"]})
    print("POS_LOCAL_OUTCOME", ranking["outcome"], decision["next_action"])
...
def run_node(node, jobs, space):
    ...
    bundle = {"discovery_id": POS_ID, "node": node["name"], "jobs": jobs}
    dispatch_path = os.path.join(OUT, "jobs", "dispatch_%s.json" % node["name"])
    dump_json(dispatch_path, bundle)
    space_path = os.path.join(OUT, "POS_SEARCH_SPACE_V1.json")
    for attempt in (1, 2):
        client = None
        try:
            client = connect(node["host"])
            ...
            sftp.put(space_path, REMOTE + "/contracts/POS_SEARCH_SPACE_V1.json")
            ...
            sftp.close()
            cmd = (
                "cd %s && PYTHONPATH=%s python3 research_engine/positioning/node_eval.py "
                "--node %s --jobs-dir %s/jobs --out %s/out "
                "--contracts-file %s/NODE_JOBS.json "
                "--space-file %s/contracts/POS_SEARCH_SPACE_V1.json"
...
def main(argv=None):
    ...
    if args.mode in ("full", "xavier"):
        ...
        if [p for p in pre if not p.get("ok")]:
            print("POS_PREFLIGHT_FAIL using local-full as authority")
            return 0
        ...
        if primary:
            ...
            dump_json(os.path.join(OUT, "POS_RANKING_V1.json"), ranking)
            print("POS_OUTCOME", ranking["outcome"], "cross_check", ranking.get("cross_check_match"))
    elif ranking is not None:
        print("POS_OUTCOME_LOCAL", ranking.get("outcome"))
    return 0
