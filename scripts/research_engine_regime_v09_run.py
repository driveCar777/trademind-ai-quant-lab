#!/usr/bin/env python3
"""Windows dispatcher for Regime Transition V0.9. Xavier only executes locked jobs."""
...
from research_engine.regime_transition import LOCKED_HASH, PARENTS, RT_ID
from research_engine.regime_transition.contract import assert_job_contract, assert_search_space
from research_engine.regime_transition.hypothesis_runner import run_hypothesis, run_path_benchmarks
from research_engine.regime_transition.jobs import build_all_jobs
from research_engine.regime_transition.prepare import prepare_pack
from research_engine.regime_transition.rank import rank_program
from research_engine.regime_transition.report import decide, write_outputs
from research_engine.regime_transition.space import build_search_space, hypothesis_map
from research_protocol.hashing import canonical_hash

USER = os.environ.get("TRADEMIND_XAVIER_USER", "dji")
PASSWORD = os.environ.get("TRADEMIND_XAVIER_PASSWORD")
REMOTE = "/tmp/tm-regime-v09"
MARKET = os.path.join(ROOT, "data", "market", "immutable")
OUT = os.path.join(ROOT, "data", "market", "research_engine", "regime_transition")
...
def lock_space():
    path = os.path.join(OUT, "REGIME_TRANSITION_SEARCH_SPACE_V0.9.json")
    if os.path.exists(path):
        ...
        print("RT_SPACE_REUSE", space["search_space_hash"])
        return space
    space = build_search_space()
    assert_search_space(space)
    write_once(path, space)
    print("RT_SPACE_LOCK", space["search_space_hash"])
    return space
...
def local_run(space, packed, iters_boot, iters_perm, tag):
    ...
    benches = run_path_benchmarks(packed)
    ranking["path_benchmark"] = benches
    ranking["search_space_hash"] = space["search_space_hash"]
    ranking["mode"] = tag
    dump_json(os.path.join(OUT, "REGIME_SMOKE_RANKING.json" if tag == "smoke" else "REGIME_LOCAL_RANKING.json"), ranking)
    decision = decide(ranking)
    write_outputs(OUT, ranking, decision, benchmarks=benches, hashes={"search_space_hash": space["search_space_hash"]})
    print("RT_LOCAL_OUTCOME", ranking["outcome"], decision["next_action"])
    return ranking, decision, benches
...
def preflight(node):
    ...
    try:
        ...
        code, out, err = ssh_run(client, "python3 -V; nproc; df -h /tmp | tail -1; echo SSH_OK", 30)
        info["exit_code"] = code
...
def run_node(node, jobs, space):
    ...
    bundle = {"discovery_id": RT_ID, "node": node["name"], "jobs": jobs}
    dispatch_path = os.path.join(OUT, "jobs", "dispatch_%s.json" % node["name"])
    dump_json(dispatch_path, bundle)
    space_path = os.path.join(OUT, "REGIME_TRANSITION_SEARCH_SPACE_V0.9.json")
    for attempt in (1, 2):
        client = None
        try:
            client = connect(node["host"])
            ...
            sftp.put(space_path, REMOTE + "/contracts/REGIME_TRANSITION_SEARCH_SPACE_V0.9.json")
            ...
            sftp.close()
            cmd = (
                "cd %s && PYTHONPATH=%s python3 research_engine/regime_transition/node_eval.py "
                "--node %s --jobs-dir %s/jobs --out %s/out "
                "--contracts-file %s/NODE_JOBS.json "
                "--space-file %s/contracts/REGIME_TRANSITION_SEARCH_SPACE_V0.9.json"
...
def main(argv=None):
    ...
    parser.add_argument("--mode", choices=["lock", "smoke", "full", "local-full", "rank", "xavier"], default="full")
    args = parser.parse_args(argv)
    ensure_dirs()
    print("RT_START", now(), args.mode)
    space = lock_space()
    packed, space = prepare_pack(MARKET, OUT, space=space)
    jobs_payload = lock_jobs(space)
    if args.mode == "lock":
        print("RT_LOCK_ONLY", space["hypothesis_count"])
        return 0
    if args.mode == "smoke":
        local_run(space, packed, 40, 40, "smoke")
        return 0
    if args.mode == "local-full":
        local_run(space, packed, 2000, 2000, "local-full")
        return 0
    ranking = None
    decision = None
    benches = None
    if args.mode in ("full", "xavier"):
        ranking, decision, benches = local_run(space, packed, 2000, 2000, "local-full")
        print("RT_PREFLIGHT")
        pre = []
        for node in NODES:
            info = preflight(node)
        ...
        if [p for p in pre if not p.get("ok")]:
            print("RT_PREFLIGHT_FAIL using local-full as authority")
            return 0
    ...
    if primary:
        ...
        decision = decide(ranking)
        write_outputs(OUT, ranking, decision, benchmarks=benches, hashes=hashes)
        dump_json(os.path.join(OUT, "REGIME_RANKING_V0.9.json"), ranking)
        print("RT_OUTCOME", ranking["outcome"], "cross_check", ranking.get("cross_check_match"))
    elif ranking is not None:
        print("RT_OUTCOME_LOCAL", ranking.get("outcome"))
    return 0
