#!/usr/bin/env python3
"""CROSS_METAL V1 dispatcher. Env password only. Does not invent IDs."""
...
from research_engine.cross_metal import LOCKED_HASH, PARENTS, CM_ID
from research_engine.cross_metal.contract import assert_job_contract, assert_search_space
from research_engine.cross_metal.hypothesis_runner import run_hypothesis
from research_engine.cross_metal.jobs import build_all_jobs
from research_engine.cross_metal.prepare import prepare_pack
from research_engine.cross_metal.rank import rank_program
from research_engine.cross_metal.report import decide, write_outputs
from research_engine.cross_metal.space import build_search_space, hypothesis_map
...
OUT = os.path.join(ROOT, "data", "market", "research_engine", "cross_metal")
...
def lock_space():
    path = os.path.join(OUT, "CM_SEARCH_SPACE_V1.json")
    if os.path.exists(path):
        ...
        print("CM_SPACE_REUSE", space["search_space_hash"])
        return space
    space = build_search_space()
    assert_search_space(space)
    write_once(path, space)
    print("CM_SPACE_LOCK", space["search_space_hash"])
    return space
...
def local_run(space, packed, iters_boot, iters_perm, tag):
    ...
    for hid in space["hypothesis_ids"]:
        ...
        out = run_hypothesis(
            hmap[hid],
            packed,
            seed=int(space.get("seed") or 20260829),
    ...
    ranking["mode"] = tag
    dump_json(os.path.join(OUT, "CM_SMOKE_RANKING.json" if tag == "smoke" else "CM_LOCAL_RANKING.json"), ranking)
    decision = decide(ranking)
    write_outputs(OUT, ranking, decision, hashes={"search_space_hash": space["search_space_hash"]})
    print("CM_LOCAL_OUTCOME", ranking["outcome"], decision["next_action"])
...
def run_node(node, jobs, space):
    ...
    bundle = {"discovery_id": CM_ID, "node": node["name"], "jobs": jobs}
    dispatch_path = os.path.join(OUT, "jobs", "dispatch_%s.json" % node["name"])
    dump_json(dispatch_path, bundle)
    space_path = os.path.join(OUT, "CM_SEARCH_SPACE_V1.json")
    for attempt in (1, 2):
        client = None
        try:
            client = connect(node["host"])
            ...
            sftp.put(space_path, REMOTE + "/contracts/CM_SEARCH_SPACE_V1.json")
            for dataset_id in list(PARENTS):
            ...
            sftp.close()
            cmd = (
                "cd %s && PYTHONPATH=%s python3 research_engine/cross_metal/node_eval.py "
                "--node %s --jobs-dir %s/jobs --out %s/out "
                "--contracts-file %s/NODE_JOBS.json "
                "--space-file %s/contracts/CM_SEARCH_SPACE_V1.json"
...
def main(argv=None):
    ...
    if args.mode in ("full", "xavier"):
        ...
        if [p for p in pre if not p.get("ok")]:
            print("CM_PREFLIGHT_FAIL using local-full as authority")
            return 0
        ...
        if primary:
            ...
            dump_json(os.path.join(OUT, "CM_RANKING_V1.json"), ranking)
            print("CM_OUTCOME", ranking["outcome"], "cross_check", ranking.get("cross_check_match"))
    elif ranking is not None:
        print("CM_OUTCOME_LOCAL", ranking.get("outcome"))
    return 0
