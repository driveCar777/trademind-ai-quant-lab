#!/usr/bin/env python3
"""VOLUME_PRICE_FLOW V1 dispatcher. Env password only. Never ships the Databento key."""
...
from research_engine.futures_vol import DATASET_ID, LOCKED_HASH, VOL_ID
from research_engine.futures_vol.contract import assert_job_contract, assert_search_space
from research_engine.futures_vol.hypothesis_runner import run_hypothesis
from research_engine.futures_vol.jobs import build_all_jobs
from research_engine.futures_vol.prepare import prepare_pack
from research_engine.futures_vol.rank import rank_program
from research_engine.futures_vol.report import decide, write_outputs
from research_engine.futures_vol.space import build_search_space, hypothesis_map
from research_engine.io_util import dump_json, load_json, write_once
from xavier_auth import xavier_password, xavier_user


USER = xavier_user()
REMOTE = "/tmp/tm-futvol-v1"
MARKET = os.path.join(ROOT, "data", "market", "immutable")
OUT = os.path.join(ROOT, "data", "market", "research_engine", "futures_vol")
...
def lock_space():
    path = os.path.join(OUT, "VOL_SEARCH_SPACE_V1.json")
    if os.path.exists(path):
        ...
        print("VOL_SPACE_REUSE", space["search_space_hash"])
        return space
    space = build_search_space()
    assert_search_space(space)
    write_once(path, space)
    print("VOL_SPACE_LOCK", space["search_space_hash"])
    return space
...
def local_run(space, packed, iters_boot, iters_perm, tag):
    ...
    ranking["mode"] = tag
    dump_json(
        os.path.join(OUT, "VOL_SMOKE_RANKING.json" if tag == "smoke" else "VOL_LOCAL_RANKING.json"),
        ranking,
    )
    decision = decide(ranking)
    write_outputs(OUT, ranking, decision, hashes={"search_space_hash": space["search_space_hash"]})
    print("VOL_LOCAL_OUTCOME", ranking["outcome"], decision["next_action"])
...
def run_node(node, jobs, space):
    ...
    bundle = {"discovery_id": VOL_ID, "node": node["name"], "jobs": jobs}
    dispatch_path = os.path.join(OUT, "jobs", "dispatch_%s.json" % node["name"])
    dump_json(dispatch_path, bundle)
    space_path = os.path.join(OUT, "VOL_SEARCH_SPACE_V1.json")
    for attempt in (1, 2):
        client = None
        try:
            client = connect(node["host"])
...
def main(argv=None):
    ...
    ensure_dirs()
    print("VOL_START", now(), args.mode)
    space = lock_space()
    packed, space = prepare_pack(MARKET, OUT, space=space)
    jobs_payload = lock_jobs(space)
    if args.mode == "lock":
        print("VOL_LOCK_ONLY", len(space.get("hypothesis_ids") or []))
        return 0
    if args.mode == "smoke":
    ...
    if args.mode in ("full", "xavier"):
        ...
        if [p for p in pre if not p.get("ok")]:
            print("VOL_PREFLIGHT_FAIL using local-full as authority")
            return 0
        ...
        if primary:
            ...
            print("VOL_OUTCOME", ranking["outcome"], "cross_check", ranking.get("cross_check_match"))
    elif ranking is not None:
        print("VOL_OUTCOME_LOCAL", ranking.get("outcome"))
    return 0
