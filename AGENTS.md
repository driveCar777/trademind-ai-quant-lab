...
**V31 中国期货截面（2026-09-06 10:10，冻结）。** 同合约展期调整 + LS 三分位只算一次 → `CN_FUTURES_XS_V31_NO_CANDIDATE`（G1/G4 资金正，G2 t 0.81，G3 3/5）。不调、不映射黄金原油。**V35 A股→商品（2026-09-06 10:25，冻结）。** CU/AG/AU/SC 篮子择时只算一次 → `CN_INFO_TO_COMMODITY_V35_NO_CANDIDATE`（验证 8 期永远做多 = 死拿；研究择时弱于 EW4）。该层关闭。合法新 D1 对象 = 0。组合仍 1 袖（ML1 V26.8）。不编新家族凑进度。
