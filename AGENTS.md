...
**V31 中国期货截面（2026-09-06 10:10，冻结）。** 同合约展期调整 + LS 三分位只算一次 → `CN_FUTURES_XS_V31_NO_CANDIDATE`（G1/G4 资金正，G2 t 0.81，G3 3/5）。不调、不映射黄金原油。**V35 A股→商品（2026-09-06 10:25，冻结）。** CU/AG/AU/SC 篮子择时只算一次 → `CN_INFO_TO_COMMODITY_V35_NO_CANDIDATE`（验证 8 期永远做多 = 死拿；研究择时弱于 EW4）。**V36 行业→期货（2026-09-06 11:40，冻结）。** 行业桶收益映射到 72 品种截面只算一次 → `CN_INDUSTRY_TO_FUTURES_V36_NO_CANDIDATE`（验证 LS −11.6% t −1.14；毛价差 t −0.89；研究 −30.7%；滚动 2/5）。层关闭。组合仍 1 袖（ML1 V26.8）。不编新家族凑进度。不改映射/特征/窗口。
