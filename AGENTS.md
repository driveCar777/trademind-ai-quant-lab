...
**V31 数据冻结 + V33（2026-09-06 09:20）。** `tm-cnfut-SINA-D1-20260906-000001`：73 品种连续 2005+ / 5,614 逐合约 2018-05+，0 失败；V31 合同待写。**V33 涨停事件模型**（用户要求的"事先预测谁会涨停"小资金版，只算一次，ATLAS 77）：前 10% 命中 12.4% vs 基础 4.4%（t 60.9）**能预测**；外壳账本验证 **−88%**、¥1M 大账本 −66% **不能赚钱** → `A_SHARE_LIMITUP_EVENT_V33_PREDICTIVE_BUT_NOT_TRADABLE_AT_20K`。彩票效应，不是资金规模问题。**禁止**调阈值/持有期/特征、日内变体、为 G1 找账本、把 ML3 塞进 ML1、重开短线/涨停/题材类。
...
**V26.6 补满仓位（2026-09-06 09:25，采纳）。** 事前算术：V26.5 现金闲置 42%（¥2,000 整手余数）。只加第二轮"同一批名字按分数每次加 1 手到 80% 预算"，只算一次 → 验证 **+33.9%（CAGR 12.3%，MaxDD −11.4%，t 2.56）** vs V26.5 +12.1%；研究 +318%（MaxDD −45%）。`daily.py` 默认 `ML1_EQMONEY_80PCT_TOPUP_MAIN`。不是 alpha 改进，是钱投出去了。**禁止** V26.7 试别的补仓/单位/仓位；因回撤加止损（V34）；改 ML1；读近期窗。
