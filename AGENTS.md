...
**V31 中国期货截面（2026-09-06 10:10，冻结）。** 同合约展期调整 + LS 三分位只算一次 → `CN_FUTURES_XS_V31_NO_CANDIDATE`（G1/G4 资金正，G2 t 0.81，G3 3/5）。不调、不映射黄金原油。**V35 A股→商品（2026-09-06 10:25，冻结）。** CU/AG/AU/SC 篮子择时只算一次 → `CN_INFO_TO_COMMODITY_V35_NO_CANDIDATE`（验证 8 期永远做多 = 死拿；研究择时弱于 EW4）。**V36 行业→期货（2026-09-06 11:40，冻结）。** 行业桶收益映射到 72 品种截面只算一次 → `CN_INDUSTRY_TO_FUTURES_V36_NO_CANDIDATE`（验证 LS −11.6% t −1.14；毛价差 t −0.89；研究 −30.7%；滚动 2/5）。**V37 仓单（2026-09-06 12:00，冻结）。** 上期所/能源注册仓单 22 品种只算一次 → `CN_FUTURES_WAREHOUSE_V37_NO_CANDIDATE`（验证 LS +41.4% t 2.19 过；毛价差 t 2.41 不过；研究 −7.7%；滚动 3/5；vs ML1 corr 0.41）。不降 t 门、不加郑商所/大商所。组合仍 1 袖（ML1 V26.8）。不编薄家族。
...
**用户 11:53 纠正（硬性）。** A 股 V29 纸面路径没搞好之前，禁止再换市场、再开期货家族。当前唯一工作 = `daily.py` + V26.8 SHORTLIST + `LEDGER_TOP20`。V31/V35–V37 已冻，不是下一刀。
**V38 Evolution Mission（2026-09-06 22:50，用户 22:45 授权，解除 11:53 的"只做 V29"）。** `docs/research_engine/V38_EVOLUTION_MISSION_DESIGN.md`。长期任务，跨会话。**进化对象 = ML1 之外**：S1 免费新信息层（L1 增减持 / L2 回购 / L3 解禁 / L4 业绩预告快报 / L5 大宗 / L6 基金持仓 / L7 质押 / L8 市场级散户情绪），每层一个预注册模型、跑完冻结、m 逐层计、corr vs ML1 <0.90 才算独立；S2 外壳级市场仓位规则 O1 SMA200 / O2 波动率 / O3 情绪，各测一次，采纳 = 验证 MaxDD 改善且夏普不低；S3 有独立袖才等权组合。**ML1 仍冻结**；不加个股出场；不读禁用/近期窗；不换市场；论坛帖子只当想法不当证据；付费先报价（余额 ≈$46）。每层完成即汇报 + ATLAS 行。V29 每日照跑，Stability 等 09-29。
**V38 L1 / S2（2026-09-06 23:50，冻结）。** L1 业绩预告/快报只算一次 → `A_SHARE_PREANNOUNCEMENT_V38L1_NO_CANDIDATE`（研究 t 9.6 集中 2018–21；验证 t −0.11；LO20 −20%；corr vs ML1 0.07）。S2 O1 SMA200 / O2 波动率仓位各一次 → 均 REJECT（O1 研究 MaxDD 不变、验证夏普降；O2 两门都不过）→ `EXPOSURE_OVERLAY_NO_IMPROVEMENT`。ATLAS 84。**禁止**：给 L1 加特征/缩持有期/限宇宙；试别的均线/阈值/仓位比例；组合 O1+O2。
**V38 L4 / L7 / O3 / S3（2026-09-07 02:10，S0–S2 结算，$0，ATLAS 87）。** `V38_EVOLUTION_MISSION_REPORT.md`。**L4 增减持**（东财 268k PIT 事件，7 特征）→ `A_SHARE_INSIDER_TRADES_V38L4_NO_CANDIDATE`：验证超额 +0.17%/20d t 5.9、滚动 4/5、corr vs ML1 0.21（有信息且独立）但 LO20 验证 −16%。**L7 质押**（中登周频 1.33M 行，6 特征，首预测 2016-03）→ `A_SHARE_EQUITY_PLEDGE_V38L7_NO_CANDIDATE`：V38 最强层，验证超额 +0.35%/20d t 6.6、IC 0.05、有数据滚动块 4/4、corr vs ML1 0.62，但 LO20 两期负（合格 EW 2016–18 / 2021–24 本身为负）。**L2 回购 DATA_BLOCKED**（免费表无历史）；L3/L5/L6 **DEFERRED**（空头机制/与 ML1 重叠，不为凑数跑）。**O3 韭菜指数**（融资增速+换手+涨停家数+中登新开户 → 过热减半仓）→ REJECT：两期夏普升、研究 MaxDD −56→−46，但验证 MaxDD 不变（2022–24 回撤前无过热）。**结论**：ML2F/L4/L7 三个信息层都有真边（0.2–0.4%/20d）且都过不了多头绝对收益门；缺的是账本不是信息。**S3 `V38_S3_INFO_STACK_CONTRACT.md` 只登记不跑**：32 特征 ML7（季报+预告+增减持+质押，全进不挑），只能在 V29 前向窗评估（≥24 期后读一次）；**禁止**在任何历史窗上评估堆叠。**禁止**：调 L4/L7/O3 任何阈值；把 INS_*/PL_* 塞进 ML1；按 HN20 重读；重开 L1/L4/L7/O1–O3。
**用户 09:44 全权授权 + O4/O5 + ML7 接入（2026-09-07 10:25）。** 用户："允许你干任何事，不用请示"。**O4 全球四指数联动减仓 → REJECT**（验证夏普 0.70→0.58；纳指/道指/日经/恒生同跌时 A 股小盘平均 +2.9%，不翻方向）。**O5 北向 20 日净流出减仓 → 两门都过（验证 TWR 38→42%、夏普 0.70→0.82、MaxDD −24.4→−22.3）但 NOT_DEPLOYABLE**：港交所 2024-08-19 起停发北向日频；不写 `daily.py`、不找代理。ATLAS 89。南下 NOT_APPLICABLE；北向个股（仅 2021-04→2024-08）、美联储/战争/政策/新闻 DATA_BLOCKED；散户拥挤 = V24 / O3 已读。**S2 关闭（O1–O5 各读一次）。** **ML7 影子已接入**：`research_engine/ml7_live/`，四信息层 env 重定向到 `live/info_layers/`（层包默认路径不变），32 特征、REFIT_240 同表模型、`SIGNAL_ML7_{date}.json` + `LEDGER_ML7.json` + `STATUS_ML7.json`；`ml1_live/daily.py` 第 9 步在 ML1 全部文件写完后调用，`--no-ml7` 可关，异常只记录。首信号 2026-08-28 与 ML1 重合 38.6%。**读取规则：≥24 个已结算期后读一次；之前任何 ML7 数字都不是结论；ML7 不进 SHORTLIST；不用 ML7 改 ML1。**
**纸面操作台 V2（2026-09-07 11:00，用户准备模拟盘）。** `PAPER_OPS_DESK_V2_DESIGN.md` + `paper_ops.py` + SPEC §29.5–29.8 + `paper.html`（旧页 `/paper/v1`）。策略每 21 个交易日只操作一次；平时页面 = 「今天无需操作」；卖出日开盘全卖、当晚更新出名单、次日开盘买。**没有**加仓/做 T/止损/提前卖（V33/V34）。页面按钮后台跑 `daily.py --asof <上一已完成交易日>`，只用仓库默认参数、有锁、可续跑；成交日志派生真实账户，买入手数按可用现金 − ¥200 重算。每次计算 $0。**禁止**：从页面改 `daily.py` 参数；把出场规则/择时/名单微调塞进 `daily.py`；用 `--force-score` 名单当当期持仓。Stability 至 09-29。**V2.1（12:05，用户反馈）**：顶栏一个开关「我的模拟账户 / 模型影子账本」，整页跟着切，两条链永不混算（`plans.actual/model`、`history_actual/model`）；我的账户只按成交日志，没登记 = 空仓、买 3/10 只就按 3 只（卖出清单/历史都是 3 只）；空仓期中同时给 A 等下一买入日（合同路径）和 B 本期名单按现金跟买（标「中途进场未检验」，卖出日不变），选择权在用户。**不要**把中途进场写成推荐或把它拿去回测当新合同。
**冻结 + 分支 + V26.9（2026-09-07 22:15，用户 21:59 要求）。** 主线冻结标签 `freeze-20260907-paper-ops-v2.1`。分支 `exp/sz-main-shell` 在独立目录 `D:\AGXXAIVER-4-WINDOWS-1-STOCK-SZ`（缓存/分数/venv 联接主库，`live/` 小拷贝），Master **:9001**（`TRADEMIND_PAPER_READONLY=1`，更新按钮拒绝；BaoStock 登录只从 :9000 发）。V26.9 = V26.8 外壳只换板块 `SZ_MAIN`（`sz.00`），只算一次：验证 +94.1%（t 2.92，17/29，逐日 MaxDD −26%）vs MAIN +39.0%（t 2.86）；研究 +609% vs +551%；差距几乎全是 2022 一年 → `VIABLE_HISTORICAL`，**同一水平，不是深市更好的证据**。主线 `daily.py` 不改（板块由权限定）。:9001 出深市影子名单（`derive_sz_shortlist.py` 从主线 SIGNAL 派生），≥24 期后读一次。ATLAS 90。**22:20 用户要求删分支**：worktree / 分支 / :9001 已删，证据搬回主线（`V26_9_*` 合同与判决、`cn_a_share_ml_v26sz/…READ.json`、`sz_main_book.py`）。**禁止**：读 SH_MAIN 单独一条；为 2022 找解释；因结果换主线板块；对 SZ/SH 读禁用窗或近期窗（"最近一个月/一年"不可回测，只看 V29 影子账本）。
