"""Smoke: GOLD H1 Ridge. Does not touch :9000 or V1/V5 READ."""
...
from research_engine.hot_mt5_gold_h1_v6 import engine  # noqa: E402
from research_engine.hot_mt5_gold_h1_v6 import paths as v6paths  # noqa: E402
...
def main():
    ...
    check(engine.RIDGE_ALPHA == 1.0, "alpha frozen at 1")
    check(engine.PROFILE == "HOT_MT5_GOLD_H1_V6_RIDGE", "profile id")
    summary = engine.run()
    check(summary.get("ok") is True and summary.get("candidate") is False, "runs, not Candidate")
    check(summary.get("model", {}).get("type") == "Ridge", "Ridge model")
    check((res / "READ.json").is_file() and (res / "FOLDS.json").is_file(), "writes V6 dir + folds")
    check("true_in_sample" in summary, "reports train-set fit")
    ...
    check((v5.read_bytes() if v5.is_file() else b"") == v5_b, "does not rewrite V5 READ")
    ...
    print("SMOKE 38 gold H1 V6:", "PASS" if not FAILED[0] else "FAIL %d" % FAILED[0])
