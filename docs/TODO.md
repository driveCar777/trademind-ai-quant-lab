...
> **最后更新:** 2026-09-14（Phase 3 计划 `AUDIT/MT5_PHASE3_ALPHA_DISCOVERY_PLAN.md`。Case File `AUDIT/MT5_STRATEGY_CASE_FILE.md`。Phase 2：`AUDIT/PHASE2_FINAL_REPORT.md`。D1 买持有净 +171%、H1 永远多净 −30%。Grok 默认不发单。不调参。`:9000` 冻结）
> **下一入口:** 冻结纸面台 `http://127.0.0.1:9000/paper`。热台 `http://127.0.0.1:9001/paper`（顶栏：今日/持仓/日程/MT5/设置）。Phase 3 实验未跑；缺事件日历与 TIPS。不是 Logistic、不是 FINAL OOS。
...
| V26.8 单位随权益增长 | {10,20,40} 研究期按夏普选 → N10；验证 TWR +39.0%（CAGR 14.0%）、IRR 10.4%、¥74k→¥86.9k；逐日 MaxDD 研究 −56% / 验证 −24% → VIABLE，采纳为 daily.py 默认 | ✅ 外壳冻结 | 2026-09-06 |
| V26.7 满仓 + 月定投 2k | 用户事前约束；验证 TWR +27.0%（CAGR 10.0%，MaxDD −13.2%，t 3.58）、¥74k→¥79k IRR 4.2%；研究 ¥236k→¥432k IRR 11.2% → VIABLE，采纳为 daily.py 默认 | ✅ 外壳冻结 | 2026-09-06 |
| V26.6 补满仓位 | 现金闲置 42%→21%；验证 +33.9%（CAGR 12.3%，MaxDD −11.4%，t 2.56）vs V26.5 +12.1% → VIABLE，采纳为 daily.py 默认 | ✅ 冻结外壳；不再 V26.7 | 2026-09-06 |
| V34 出场规则诊断 | 13 条止盈/止损/移动/保本，只读研究期：ML1 11/12 更差、TP20 略好为噪音不选；V33 无一翻正 → EXIT_RULES_NO_IMPROVEMENT | ✅ 冻结；不写进 daily.py | 2026-09-06 |
...
| V31 CN Futures XS | 合同先提交；同合约 20 日 LS 三分位只算一次：验证 LS +6.0% t 0.48（G1 过）、毛价差 t 0.81（G2 不过）、滚动 3/5（G3 不过）→ NO_CANDIDATE；ATLAS 78 | ✅ 冻结；不调、不映射 MT5 黄金原油 | 2026-09-06 |
...
| V37 WAREHOUSE | 上期所/能源仓单 22 品种只算一次：验证 LS +41.4% t 2.19（G1 过）、毛价差 t 2.41（G2 不过）、研究 −7.7%、滚动 3/5、vs ML1 corr 0.41 → NO_CANDIDATE；ATLAS 81 | ✅ 层关闭；不降门；不加交易所 | 2026-09-06 |
...
| V26.3 ML1 ONELOT 70PCT MAIN | 用户 23:36 纠正（≤¥100、70% 总仓、每只 1 手、N 随价格）→ V26.2 作废；V26.3 只算一次：验证 −10.8%、超额 t 0.87 → **NOT_VIABLE**（执行外壳问题，不调）。`daily.py` 默认切到一手制；`OWNER_CONSTRAINTS_AND_GOAL.md` 复利算术：缺口是本金不是边 | ✅ 合同+单次读取+管线+目标文档；人暂不上实盘；影子账本 ≥6 期后只看"外壳是否如预期吃边" | 2026-09-05 |
| V26.2 近期窗诊断 | 2024-03→2026-08 已读窗：+31.5% 但超额 ≈0 = 纯 β；近 6 月 −12.6%。无决策，前向影子账本说话 | ✅ 报告 | 2026-09-05 |
| V26.2 ML1 TOP10 20K MAIN | 用户事实：仅主板、¥20k、≤¥20 → N=10 算术推出。历史 VIABLE（验证 +32.7%，MaxDD −11%）。`daily.py` 默认已切换；每日 `SHORTLIST` 10 只 | ✅ 合同+读取+管线；⏳ 前向影子账本 ≥3 期后用户自决是否手工 Paper | 2026-09-05 |
