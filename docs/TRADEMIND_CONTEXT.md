...
> **版本:** V1.0  
> **更新时间:** 2026-09-06 11:53（用户纠正：A 股 V29 未完工禁止换市场；期货 V31/V35–V37 已冻不是下一刀）  
...
- **纸面台 FACT 2026-09-06 12:20：** 前端 `dashboard/paper.html`，Master `GET /paper` + `GET /api/v1/paper/desk` + `POST /api/v1/paper/preview` + `PUT /api/v1/paper/settings`（SPEC §29）。色板同 Stitch 炒股绑定（无酒红）。资金/定投/价格上限/N 可调 = 用最近 SIGNAL 重算手数，不改 ML1、不发单。Stitch MCP 本轮未挂上。
- **纸面更新日历/停止 FACT 2026-09-07 20:40（用户：休市应按最近开盘日；已拉过要说；漏票要提示；卡住 111 分钟没法停）：** 卡住的 pid 8232（`--asof 2026-09-07`）已 `taskkill`。周一 18:00 后 asof=当天是对的；周六 9/6 才会回跳到 9/4。`POST /api/v1/paper/update/stop`；已是最新且无缺口 → `skipped` 不 spawn；缺口写 `补漏 N 只`。BaoStock socket 40s → `BAR_HANG` 跳过。不自动重跑。ML1 参数不动。
- **:9001 自有池 FACT 2026-09-13：** `:9000` 与 `:9001` 池分开。热台权威名单 = `live/paper_hot/POOL.json`，**永不写** `:9000` SIGNAL/SHORTLIST。周一 `asof_session>=2026-09-14` 才启用；现在持仓维持。合适就买当前热台池；不合适刷新热台池（只读 ML1 SIGNAL 当井，本场不二次问 Grok，每天最多刷 2 页）。不是 Candidate。
- **MT5 黄金 H1 FACT 2026-09-13：** 用户计划是小时/15/30 分钟交易。V1–V5 是日线。H1 数据：45,818 根，买持有 +250% / CAGR 17.5%。成交对账 1752/1752 无错。**真样本内已补测**（`HOT_MT5_GOLD_H1_TRAIN_VAL_REGIME.md`）：V1 训练 IC **0.517** / R² 0.187 / 命中 62%；44 折训练 IC 0.619 vs 测试 IC **0.038** → **过拟合**，不是死特征、不是常数分。此前把 research_70 −46% 当成「样本内」是错的（那是 walk-forward 样本外）。状态：COVID/2022/2023 净负；2024-05→2026-09 净 +29% 但买持有 +90%，不晋升。**V6 Ridge**（α=1）训练 IC 0.055 / 折外 0.007 / 净 −52% / 验证 t 0.20。**V1** 树净 −35%。**V2** ORB −74%。**V3** 昨日高低仅验证过门。**V4** 亚洲反向 −80%。**V5** 小时钟净 −67%。下一刀须换问题，禁止再堆 24h `sign` 的树。ATLAS 102。
- **MT5 黄金 H1 V7/V8/V9 FACT 2026-09-13：** V7 train IC 0.54 / fold OOS −0.001, net −37%, val t 0.16。V8 3-class train acc 65% / fold OOS IC 0.007, val −72% t −4.3, CASH 2.4%。V9 5-col train IC 0.283 / fold OOS −0.001, net −52%, val t −0.03。Trio: changing label or dropping columns does not fix overfit on own-price H1。`candidate=false`。Stop stacking 24h sign / more trees / IC-picked features。ATLAS 105。
- **Phase 2 Ava GOLD FACT 2026-09-13：** 地基 + 第一批研究。SPEC §30。Grok 默认不发单（`TRADEMIND_HOT_GROK_SEND=0`）。活终端快照：杠杆 **400**、GOLD 合约 100、点差 34、swap −1.54/+0.64。D1 RESEARCH 买持有净 **+171%** / 日线盯市 CAGR **13.2%** / 月中位 **+1.11%** / **0/79** 月 ≥20%；H1 永远多净 **−30%**。EXP-001 Linear 只赢 Naive、输买持有 → 停阶梯。EXP-002 只跑基线。EXP-003 只登记。`candidate=false`。20%/月 **UNSUPPORTED**。报告 `AUDIT/PHASE2_FINAL_REPORT.md`。不改 `:9000` / ML1。不重开 D1 V1–V5 / H1 V1–V9。
- **MT5 取证审计 FACT 2026-09-13：** 不是一套自动交易系统。层 A=`paper_hot_mt5` Grok 市价（可 demo 发单、无 SL/TP、分数不进提示词）；层 B=V9 RSI 人手确认；层 C=D1/H1/V30/V32 研究坟场不发单；层 D=V4 跟盘只展示。本机无 `MT5_JOURNAL`。设置手数 0.1。无法稳定盈利的主因是发单逻辑无边 + 价内 ML 无边 + V4 非独立 alpha + 标签≠利润 + 执行≠回测。报告 `STRATEGY_FORENSIC_REPORT.md` / `FAILURE_ANALYSIS.md`。SPEC §29.31。未调参。
- **MT5 黄金 V4 路径/出场诊断 FACT 2026-09-13：** 102 笔已平 + 当前未平。均 MFE +3.9%、均吐回 3.3%。47 笔亏损里 41 笔从未摸到 +3% 高点；收盘曾 ≥+3% 仍收负只有 5 笔。2026-03 大亏高点 +3.0%/收盘最高 +2.2%；2026-05 收盘从未绿过。当前未平腿 MFE 仅 +0.36%，保本开不了。保本 3% 全样本 +52%→+14%；移动 5% 全样本略好、研究窗更差。**不选、不写进跟盘。** `HOT_MT5_GOLD_V4_PATH_EXITS.md`。SPEC §29.30。
- **MT5 黄金 V4 手工跟盘 FACT 2026-09-13：** 主人要在 Ava GOLD 上赚钱。诚实路径 = 冻结 D1 12 月 TSMOM V4（`VIABLE_HISTORICAL`，验证 +84% t 2.18，MaxDD −47%）+ 可选 V5 仓位上限（MaxDD −40%），**不是** H1/M15。最后一根 D1 2026-09-11：V4 **多**，未平腿 2026-08-25 进、盯市约 −7%，坐在已诊断的 9 月初砸盘上；252 日动量仍 +7%。规格 `HOT_MT5_GOLD_FOLLOW_V4_SPEC.md`；`gold_follow/STATUS.json`；:9001 MT5 页只展示，不写 Grok，不 `order_send`。`candidate=false`。禁止再训 H1 树。ATLAS 106。

- **MT5 own-price D1 清点 FACT 2026-09-13：** 外壳五刀已冻：V1 永远在场 7/7 否；V2 成本门槛仍在场 7/7 否；V3 ATR 门槛一直空 7/7 否；V4 12 月 TSMOM 黄金 `VIABLE_HISTORICAL`（81/21 多空，空头每笔 −1.4%，最长连多 51）；V5 10% 波动倒数黄金仍历史过门（MaxDD −47→−40，验证 t 2.83），原油研究翻正验证不够。0 Candidate。清点 `HOT_MT5_OWN_PRICE_D1_INVENTORY.md`。禁止 V6 价内变体、改 252/20/10%、写 Grok。ATLAS 95。
- **MT5 长任务 V2法医+V3+V4 FACT 2026-09-13：** 用户要求持续分析、不要等一句做一句。V2 法医 11446 笔：与 V1 同向 71–90%，黄金开头再连空，外汇毛正净负，盈亏比≈1。V3 `ATR×√hold`：CASH 标签 79–88%，7/7 覆盖否 → `HOT_MT5_ATR_BARRIER_V3_NO_CANDIDATE`（禁止把 k 改小）。V4 12 月 TSMOM 无树：黄金验证 +84% t 2.18 → `VIABLE_HISTORICAL`（2018 起样本、81 多/21 空、2024–25 牛、MaxDD −47%、盈亏比 1.15），**不是 Candidate、不上 Grok**；其余 6/7 NO_CANDIDATE（欧美/美日全样本 TWR 大、CAGR 1–2%、验证≈0）。own-price D1 队列停。不要开 V5 价内变体。ATLAS 93–94。
- **MT5 成本门槛三分类 V2 FACT 2026-09-13：** `HOT_MT5_COST_AWARE_V2` 只算一次。标签 = |hold 收益| > 2×META 预期往返成本才做多/空，其余 CASH；LightGBM 三分类；argmax，CASH 次日再看。门槛事先是 10–33bp，5/10 日波动几乎总是更大 → 标签 CASH 只有 4–20%，覆盖率仍 82–91%，空仓没发生。**7/7 `NO_CANDIDATE`**（全样本/验证 TWR 全负；盈亏比 0.89–1.03）。`HOT_MT5_COST_AWARE_V2_NO_CANDIDATE`。禁止加大 λ / 改用 20bp。不覆盖 V1 `READ.json`。不写 Grok。不与 A 股对冲。ATLAS 92。见 `HOT_MT5_COST_AWARE_V2_DECISION.md`。
- **MT5 空仓/门槛设计 FACT 2026-09-13：** 乱做的主因是策略（`sign(score)` 永远在场）和训练目标（MSE 拟合约等于 0 的毛收益、没有空仓类），不是调用坏了。外汇样本够大。盈亏比 0.85–1.04，不是趋势跟踪那种 1.5–2.5。默认应空仓，开仓过 2×成本门槛；覆盖率过低直接否。禁止用已看过的 20bp 当门槛。V2 已按该设计跑完并证伪（门槛相对波动太矮）。见 `HOT_MT5_SITOUT_AND_HURDLE_DESIGN.md`。
...
- **MT5 分品种训练 FACT 2026-09-13：** 七个品种各自一份预注册 LightGBM + 持有外壳，全样本成本后回测一次。数据：GOLD/CRUDE D1 2018-12→2026-09-11（约 2410，经纪商上限）；EURUSD/USDJPY/USDCHF D1 1971 起约 1.58 万根；GBPUSD/USDCAD 1993 起约 1.01 万根。H1 已存 4.5 万–8 万根，本合同不搜。**7/7 `NO_CANDIDATE`**（全样本 TWR 全负；验证全负）。策略规则写在结果 JSON，`deploy=false`。不要用黄金 2024 / 原油 2025 年切片改参。美股仍不做。不写进 Grok 提示词。不与 A 股对冲。ATLAS 91。`:9000` 不动。
- **A 股 / MT5 隔离 FACT（用户确认，硬性）：** 禁止 A 股赚了补 MT5、两边对冲、拿两边数字改提示词。日志分开。
- **:9001 晚间必看 FACT 2026-09-13：** 能买卖时白天最多 3 次 Grok。不能买卖白天 `SKIPPED_NO_CAPACITY`，但交易日 19:30 若当天还没诊过 **必须看一次**。计划钉 `cash_policy` / `name_source` / `pool_age`。`:9000` 不动。
- **:9001 台账快照 FACT 2026-09-13：** 热台 JOURNAL 与 `MT5_JOURNAL` 每笔带 reason + snapshot。MT5 仍 08:30/20:30，不实时埋点、不连续 AI 盯仓、不按品种另训模型。不是为提高收益。不是 Candidate。`:9000` 不动。
- **:9001 多页纸面台 FACT 2026-09-12：** `paper_hot.html` 顶栏五页（今日/持仓/日程/MT5/设置），无底栏。今日=一张账户+色例+曲线卡，右「要不要动手」，下四张短卡（点开放大）。曲线蓝/橙/黄/绿/紫/青/粉 + 十字虚线；浮层只写日期和数字。`desk.equity_curve` + `GET /hot/symbol/{symbol}/curve`。默认页不展示对照账/账本1/2/N5/「跑融合台」/Layer A/B。不是 Candidate。`:9000` 不动。
- **时钟分裂 + :9001 Ava MT5 分品种台 FACT 2026-09-12 11:11：** 主人只在工作日晚上更新 A 股日线。开盘/午休/**收盘**诊股都允许 T-1 本地 + 联网；成交仍是下一开盘；`clocks.local_asof` ≠ 网上今天 ≠ `fill_date`。MT5 报价走 Ava 终端，不读 `live/bars`。`:9001` 黄金/原油/欧美/美日/美英/美加/美瑞分账，股票篮子只建议不发单；08:30 / 20:30 各 1 次 Grok，0.01 手，**仅 demo**，live 拒绝。不是 Candidate。分品种模型已另跑一次（`HOT_MT5_PER_PRODUCT_V1_NO_CANDIDATE`），不要把那些分数写进本台提示词。A 股融合台仍纸面、同花顺手工跟单。
- **实时诊股叠加 FACT 2026-09-12 10:51（用户决定，2 个月观察）：** `:9000` ML1 + V26.8 仍是唯一名字来源；`:9001` 叠 Grok 4.6 稀疏实时诊断（BUY/SELL/ADD/REDUCE/REPLACE/HOLD），场次 09:35 / 11:30 / 15:05 各最多 1 次联网调用，19:30 只结算（收盘计划缺失且行情新鲜才补；`MAX_GROK_CALLS_PER_DAY=3`，永不第 4 次）。**实时场次不用 Layer A**（账本2 +3.3% vs +39.0% 在案）。宇宙 = ML1 SHORTLIST ∪ SIGNAL 前 30 ∪ 当前持仓，池外 `OUT_OF_UNIVERSE`；ADD 须已持有、REDUCE 部分卖、REPLACE = 卖旧买池内新；预算 = 现金 + 卖出净额 − ¥200；名义 ≤ 仓位 × 权益；T+1；一手 100 股。盘中计划都等下一开盘成交（PENDING，不编价），同 fill_date 后出覆盖先出（SUPERSEDED）。任务计划 `TradeMind_HotFusionOpen/Lunch/Close/Daily` 已建；`POST /api/v1/hot/fusion/session`。**读取：≥24 已结算融合期或 2026-11-12 取晚者，只读一次；中途不为收益调提示词/阈值/场次；不是 Candidate；:9000 不动；无真实下单。** MT5 只写 `MT5_EXECUTION_VS_HOLDING.md`（更频繁交易 = 更频繁付成本；H1/tick/看完再训禁止；等新预注册合同）。
- **热台初始资金 FACT 2026-09-12 10:45（用户：:9001 初始资金 ¥20,000，看纸面成交、手工在同花顺跟单）：** `FUSION_SETTINGS.initial_capital=20000`；`paper_hot.derive_account` = 种子 + 日志流水，`seed:true` 的 DEPOSIT 只作证据不重复计。审计：日志里本来就有 09-10 ¥20,000 DEPOSIT，−¥259 不是缺种子，是 10 笔手工买入 ¥20,209 + 费 ¥50 = ¥20,259 超出 ¥20k；种子化后现金仍 **−¥259**、市值 ¥20,192、权益 ¥19,933（较初始 −¥67），买入预算 0，直到卖出。不删成交、不造反向单。**账本2 已读完 29/29**（v1.1，0 超时）：TWR **+3.3%** vs 账本1 +39.0% → 匿名 Grok 过滤在验证窗是减分不是加分；对照账保留，不是 Candidate，不调提示词。:9000 不动。
- **Hot Desk V3 自动纸面登记 FACT 2026-09-12 00:50（用户：「让他每天的建议，都纸面登记模拟买卖」）：** `paper_fusion_fill.py`（仅 :9001）。每个融合计划到 `fill_date`、`live/bars` 有该日开盘价，就按**开盘价 + `paper_ops._est_fee`** 自动把 BUY/SELL 写进热台 `JOURNAL.json`（`auto=true`、`plan_id`、`note="FUSION auto {asof}"`）；先卖后买、T+1 当日买入不可卖、买入 ≤ 现金 − ¥200 手数向下取整、未持有不卖；`FUSION_FILLS.json` 记请求/成交/跳过；幂等；开关 `FUSION_SETTINGS.auto_fill` 默认开。每日驱动 `POST /api/v1/hot/fusion/daily` = 结算 → 行情新鲜（`paper_ops.freshness` 只读）才跑管线，陈旧 `SKIPPED_STALE_DATA` 不调 Grok。任务计划 `TradeMind_HotFusionDaily` 19:30 周一至五调 `scripts/hot_fusion_daily.bat`；**不触发 :9000 更新**（仍是用户按钮）。**自动成交是模拟**：没有真实下单、没有 `order_send`；账本数字是纸面，不是 Candidate、不是承诺。不改 ML1 / V26.8 / daily.py / paper_ops.py。
- **Hot Desk V3 补充 FACT 2026-09-11 晚（匿名协议 v1.1 / N5 集中 / 账本2 状态）：** Cursor API 当晚约一半请求在 TLS 层断（`SSL UNEXPECTED_EOF`、偶发 502），`create_agent` 根本没发出去——账本2 与首次融合运行的「超时」是传输问题不是 Grok 慢。修：`cursor_cloud._request` 传输重试 5×、`wait_run` 容忍轮询失败且默认 900 s、每期重试 1 次；匿名 payload 改 **v1.1**（`[o,h,l,c]` 数组、2 位小数、索引隐含，信息不变、体积减半，泄露断言不变，`B2_LEDGER.anon_protocol` 记录；09-10 单期一枪是 v1.0）。**N5 集中合同**（`HOT_N5_CONCENTRATION_CONTRACT.md` 跑前写，`book_n5.py`，只改 `n_target=5`，只读 VALIDATION 一次）→ **`HOT_N5_CONCENTRATION_NOT_VIABLE`**：TWR −0.79% vs 账本1 +39.03%，CAGR −0.3%，逐日 MaxDD −28.1%，打过账本1 8/29 期，超额均值 −1.19%/期 t −2.03。集中不是杠杆；不试别的 N；主线 n_target=10 不动。桌面 `books.n5` + 页面第四个对照账。**真正根因（23:15）**：`GET /agents` 显示「失败」的 `POST /agents` 服务端都成功了（TLS 在读响应时断），传输重试造成重复计费 agent → `create_agent` 改为先认领同名新 agent 再重试；此后账本2 v1.1 每期正常落地（23:27 已 2/29，n_timeout 0）。**Grok 设计评审一次调用完成**（`PAPER_HOT_DESK_V3_GROK_CONSULT.md`）：采纳 `claim_class`/`priced_in` 自报 + 硬规则「已进下一开盘的信息不买不卖」、默认 HOLD、计划 `audit` 块与 `prompt_hash`；拒绝缩池到 10 只 / 收回 `exposure_pct` / 禁 SELL / 改 20 日频（用户明示要日频激进叠加，风险记为已知失败模式）。融合台仍不是 Candidate。
- **Hot Desk V3 融合台 FACT 2026-09-11（用户：三本融成一台、强云模型联网 + 本地 ML1、更激进、不碰 :9000）：** `:9001/paper` 默认视图 = 一条管线：ML1 `SHORTLIST` ∪ `SIGNAL` 前 30 为池（没有新本地模型）→ Layer A 匿名 60 根 K 线 Grok `keep[]`（禁网）→ Layer B Grok 联网给仓位/BUY/HOLD/SELL → 本地硬规则截断（下一开盘、T+1、≤8 只、名义 ≤ exposure×权益、新买 ≤ 现金−¥200、只买池内、只卖持仓）→ `live/paper_hot/FUSION_PLAN_{asof}.json`；成交仍手工登记 `paper_hot/JOURNAL.json`。三本降为「对照账」开关，不删不混算。**不是 Candidate；联网层不可回测（前视）；激进 = 换手高、费用高。** 账本2 超时修复：`GrokTimeout` → 该期 `GROK_TIMEOUT` 不计 TWR、单独计数、可续跑；29 期验证窗已重新后台起跑；读完前页面固定「账本2 未读完，匿名过滤增量未知」。Grok 设计评审一次（`PAPER_HOT_DESK_V3_GROK_CONSULT.md`），只采纳日志/提示词/风险展示类建议。SPEC §29.11；`paper_fusion.py`；冒烟 25。`:9000` / `daily.py` / `paper_ops.py` / ML1 / V26.8 未动，只重启 `:9001`。
- **Hot Desk V2 三本对照账 FACT 2026-09-10：** `:9001/paper` 大改。账本1 = 冻结 ML1+V26.8 验证回放（应对上 TWR +39.03%）。账本2 = 同一批候选只把匿名 OHLC 给 Grok（禁止搜网/代码；剩余路径识别 ≠ 零泄露；**不能当 Candidate**）。账本3 = 联网纸面，ML1 SHORTLIST 为池，下一开盘登记，日志仍 `JOURNAL.json`。不改 `:9000` / `daily.py` / ML1。SPEC §29.10。启动仍 `scripts\\start_hot_desk.bat`。
- **Hot Desk V1 FACT 2026-09-10（用户：上一版冻结；新版 :9001/paper；不定 20 日；Cursor 联网找热点/龙头/做T/定仓；密钥 D:\\Cursor\\APIKey.txt）：** `:9000/paper` V2.1 不动。新进程 `hot_main` `:9001`，日志 `live/paper_hot/JOURNAL.json`。Cursor Cloud Agent **不传 repos**（无仓库，不改研究库）。下拉 `GET /v1/models`。普通账户 T+1。不发单、不改 `daily.py`、不是 Candidate。V33/V34 证伪仍在：这是授权的高风险试验。启动 `scripts\\start_hot_desk.bat`。§29.9 保留为 V1 失败记录；页面语义已被 V2 替换。
- **Hot Desk brief 失败 FACT 2026-09-10（用户：点出简报一直出不来，是不是 Grok Extra High Fast 名字错了）：** 不是。官方 id 就是 `grok-4.6`（displayName Cursor Grok 4.6）。Extra High Fast = `effort=xhigh` + `fast=true`，不是另一个模型。失败原因是 `POST /v1/agents` 连不上，然后误走死掉的 Clash `:7890`（`10061`），页面又没显示 `BRIEF_RUN.error`。已：跳过未监听的 7890、下拉列出 Extra High/Fast、失败红字。
- **Hot Desk brief 卡死 FACT 2026-09-10（用户：简报十几分钟还没好）：** 不是 Cursor 在算。后台写 `BRIEF_RUN.json` 时 Windows `os.replace` WinError 5，线程已死，页面还以为 `running`。已：重试写入、线程死后清 `running`。
- **纸面影子收盘 + AI 目录 FACT 2026-09-10（用户：影子账本跟模拟账户对不上、影子收盘不更新；模型不要只本地、要 Cursor/DeepSeek 下拉）：** 对不上允许：影子 = V26.8 全买 + 模型开盘价/佣金；模拟账户 = 成交日志（买几只算几只、你填的价和费）。影子收盘以前用 `LEDGER_TOP20.json` 的 `mark_close`（上次 `daily.py` 快照），所以会停在例如 09-08，而模拟账户已经读 `live/bars` 的 09-09。现 `ops()` 两边都用 `_last_close`。控制台模型改为 `<select>` 目录 id（`local:` / `deepseek:` / `cursor:`），禁止手输。DeepSeek key → 官方 `/models` + `/chat/completions`。Cursor key → `GET /v1/models` 只填下拉；Cursor HTTP API 是 Agent 不是聊天补全，选了 503。密钥只进 `.env`。
- **纸面操作台 V2.1 FACT 2026-09-07 12:05（用户 11:48/12:02 反馈：影子账本与模拟账户混在一起误导；要能只买几只；历史随账户切；"只有月初才能买吗"）：** `ops()` 返回 `plans.{actual,model}` 与 `history_actual/history_model`，两条链永不混算；页面顶栏一个全局开关驱动整页。**我的模拟账户** = 只按成交日志：没登记 = 空仓（不再回退到模型账本）；买 k/N 只 → 持仓、卖出清单、每一期都按 k 只并提示；现金 0 提示先入金；期中空仓 → `NO_POSITION/WAIT`，同时给 **A** 等下一买入日（合同路径）与 **B** 本期名单按现金重算手数、金额按最新收盘、标「中途跟买（未检验）」、卖出日不变（`mid_entry_option`），由用户选。**模型影子账本** = V26.8 官方（全买、¥20,000、月定投）只读，不受用户行为影响。`history_actual` 状态 `OPEN|CLOSED|NOT_TRADED|PENDING_ENTRY`。周期是每 21 个交易日，不是月初。Smoke 22 共 25 项 PASS。**禁止**：把中途进场写成推荐/回测成新合同；让两条链互相影响；用用户的部分成交改模型账本。
- **纸面操作台 V2 FACT 2026-09-07 11:00（用户 10:24 准备操作模拟盘，提出日常使用问题）：** 设计与逐条回答 `docs/research_engine/PAPER_OPS_DESK_V2_DESIGN.md`；`master/api/app/service/paper_ops.py`；SPEC §29.5–29.8；`dashboard/paper.html` 重写，旧页 `paper_v1.html` = `/paper/v1`。**产品事实**：策略每 21 个交易日只操作一次（信号日收盘出名单 → 次日开盘买 → 持 20 日 → 卖出日开盘全卖 → 当晚出新名单，卖与买隔一天），中间每天页面都是「今天无需操作」；**没有**加仓/做 T/止损/提前卖（V33/V34）；非信号日运行不换名单（`--force-score` 的 09-04 名单标为 `PREVIEW_NON_CHAIN`）。**接口**：`GET /api/v1/paper/ops`（`freshness` 上一已完成交易日 = 今天仅在 18:00 后；`plan.phase ∈ HOLD | SELL_TODAY | SIGNAL_TONIGHT | LIST_READY_BUY_TOMORROW | BUY_TODAY | NO_POSITION`；卖出清单 = 成交日志持仓，无日志退回模型 fills；买入名单按可用现金 − ¥200 用 `preview_lots` 重算；警告：上期未登记卖出 / 本月 ¥2,000 定投未登记 / 数据落后 / 上次运行失败）；`POST /api/v1/paper/update` 后台跑 `daily.py --asof <上一已完成交易日>`（仓库默认参数、锁 `live/runs/CURRENT.json`、日志 `live/runs/RUN_*.log`、psutil/tasklist 判活、`HISTORY.json`）；`GET/POST/DELETE /api/v1/paper/journal` 追加式事件 `live/paper/JOURNAL.json`（BUY/SELL/DEPOSIT/WITHDRAW/NOTE；FIFO 派生持仓与现金；手续费空则估 ¥5 起 0.03%，卖出加印花税 0.05%；市值取 `live/bars` 最新收盘）。**运行纪律**：收盘后 18:30 以后更新，忘了次日 08:30 前补；重复点无害（有锁）；中断再点续跑；隔天补齐所有缺失交易日；新闻不是特征；季报/预告/增减持/质押是 ML7 输入已并入同一次运行；**每次计算 $0**（BaoStock/东财/中登免费，Databento 只用于期货）。**已知怪癖**：V26.8 冻结算术 `n=floor(capital/unit)`，`unit=capital/n_target` 时浮点可得 9 而非 10（官方 `shortlist.py` 同样，不改）。**不改** `daily.py` 参数、ML1、V26.8。Smoke：8 场景状态机模拟 + HTTP + 浏览器 + 页面触发后台更新；Stability 至 09-29 卖出日 / 09-30 买入日走一遍。Stitch `gen-3` 已落地（TEXT_TO_UI_PRO 仅网页可看）。
- **V26.8 复现 + 08-28 开仓市值 FACT 2026-09-06 12:05：** 冻结验证期重跑 **MATCH**（TWR +39.0298%，权益 ¥86,856.15，29 期，Δ=0）。现场仓：08-31 开盘 10/10 成交、投入 ¥19,739.72、买费 ¥50.20；市值 08-31 ¥20,094 → 09-01 ¥20,516 → 09-04 **¥20,157（+¥157 / +0.79%）**。验证 29 期：收盘估 vs 次日开盘 **名字 0 次换、手数 17/29 完全相同**，隔夜缺口均值 +0.10%。`LEDGER_TOP20` 现记开盘成交+市值。不改 ML1/V26.8。
- **V38 L1 + S2 FACT 2026-09-06 23:50：** (a) S0 `V38_HYPOTHESIS_LEDGER.md`：LSY CH-3/CH-4（异常换手 = 价格层，BLOCKED_BY_RULE）；A 股 PEAD 文献（预告 > 财报 > 快报）；A 股高管**无择时能力**（L4 降序）；回购预案脉冲；解禁负漂移；CICSI 情绪构成；社区"小市值 2024 后失效"只作风险提示。(b) **L1 业绩预告/快报**合同先冻结 → 东财 `RPT_PUBLIC_OP_NEWPREDICT` + `RPT_FCI_PERFORMANCEE` 124 期 0 失败、100,400 PIT 事件（快报 60% 行日期重标已剔）→ 9 固定特征 → 1 LGBM → **`A_SHARE_PREANNOUNCEMENT_V38L1_NO_CANDIDATE`**：研究超额 +0.25% t 9.6（2018–21 t 12–13）、验证 **−0.00% t −0.11**、LO20 验证 −20.3%、滚动 3/5、corr vs ML1 **0.07**（独立但无边）。模型最依赖公告后反应（CAR3/成交量/跳空 42%）。ATLAS 82。**禁止**加特征/缩持有期/限宇宙/换 HN20/读禁用窗。(c) **S2 O1/O2** 合同先冻结各算一次 → **`EXPOSURE_OVERLAY_NO_IMPROVEMENT`**：O1 主板 EW<SMA200→50%：研究 MaxDD **不变 −55.9%**（2015 崩盘发生在均线上方），验证 MaxDD −24→−17% 但夏普 0.70→0.52、CAGR 13.6→7.6% → REJECT；O2 波动>研究中位×1.5→50%：验证 MaxDD −24→−26%、夏普 0.63 → REJECT。ATLAS 83–84。结论同 V34：回撤是 β 集中释放，简单开关事前挡不住；正路仍是第二袖。**禁止**试别的均线/阈值/比例、组合 O1+O2。O3 待 L8。下一层 L2 回购（`RPT_WEB_GETHGLIST` 已下线，需找现行表）。组合仍 1 袖。
- **V38 L4 / L7 / O3 / S3 结算 FACT 2026-09-07 02:10（$0，ATLAS 87，`V38_EVOLUTION_MISSION_REPORT.md`）：** (a) **L4 增减持**：东财 `RPT_SHARE_HOLDER_INCREASE`（NOTICE_DATE 后首日可见）+ `RPT_EXECUTIVE_HOLD_DETAILS`（CHANGE_DATE+3 日）2007–2024 共 267,830 PIT 事件 → 7 固定特征（120 日窗、按 20 日均成交额缩放）→ 1 LGBM → **`A_SHARE_INSIDER_TRADES_V38L4_NO_CANDIDATE`**：验证超额 +0.17%/20d **t 5.9**、滚动 4/5、corr vs ML1 **0.21** / vs ML2F −0.09（有信息、独立），但 LO20 验证 **−16.2%**（原始 20 日均值 −0.56%，熊市）。增益 INS_NET_RATIO 22% / INS_NEG_SELL 18%。ATLAS 85。(b) **O3 韭菜指数**（`v38_overlay/sentiment.py`）：融资余额 20 日增速 + 合格中位换手 + 涨停家数占比 + 中登月新开户（`RPT_STOCK_OPEN_DATA` 2015-04→2023-08）各 250 日 z → 均值 > +1 → 当期 50% 仓，只读一次 → **REJECT**：研究 TWR +562→+540%、夏普 0.83→**0.88**、MaxDD −55.9→**−45.6%**；验证 TWR +38.0→+39.8%、夏普 0.70→**0.73**、MaxDD **−24.4% 不变**（门要求严格改善；2022–24 回撤前无过热）。合成 15% 时间 >+1（2013 初/2014-09→2015-06/2018-03/2019-03/2020-07/2021-08）。ATLAS 86。**不调阈值/仓位**；有真正新窗（V29 ≥2 年）可重新预注册同一条。(c) **L7 质押**：中登 `RPT_CSDC_LIST` 周频 2014-03→2024-02 共 1,434,193 行、PIT 1,331,951、508 快照；周五快照 +3 日可见、沿用 ≤30 日、缺席 = 零质押；首预测 = 层起点 + 500 日 = **2016-03-28**（首跑因 2012 起点无训练行报错，修正前无结果）→ 6 固定特征（−比例 / −Δ20 / −Δ60 / −log 笔数 / −质押市值÷成交额 / 比例×min(ret60,0)）→ 1 LGBM → **`A_SHARE_EQUITY_PLEDGE_V38L7_NO_CANDIDATE`**：研究超额 +0.38% **t 11.3**、验证 +0.35% **t 6.6**、IC 0.066/0.053、有数据滚动块 **4/4** 全正、corr vs ML1 **0.62** / vs ML2F 0.52；但 LO20 研究 **−25.0%**（MaxDD −51%，2016-11→2018-10）、验证 **−9.2%**（合格 EW 本身 −0.59%/20d）；HN20 −48% / +3%。增益 PL_NEG_MCAP 27% / PL_NEG_RATIO 25%。ATLAS 87。(d) **L2 回购 DATA_BLOCKED**：唯一在线表 `RPTA_WEB_GPHG` 仅 2,986 行（2019 全年 97）= 每方案一条最新执行，无历史；不采购。**L3 解禁 / L5 大宗 / L6 基金持仓 DEFERRED**：供给冲击是空头机制、单独多头层先验为零；基金持仓与 ML1 大盘倾斜重叠；不为凑数跑。(e) **结构结论**：ML2F（t 5.6）/ L4（t 5.9）/ L7（t 6.6）三个免费信息层验证期都有真边（0.2–0.4%/20d ≈ 2–5%/年）且都过不了 20 日多头绝对收益门；ML1 过门是因为 1.47%/20d。缺的是账本不是信息。(f) **S3 `V38_S3_INFO_STACK_CONTRACT.md` 只登记不跑**：32 特征 ML7（季报 10 + 预告 9 + 增减持 7 + 质押 6，**全进不按结果挑**，无价格特征），V25 同参数；历史全窗（含禁用窗）只作训练；**只能在 V29 前向窗评估**，≥24 个不重叠期后读一次（超额 t ≥ 2、LO20 > 0、corr vs ML1 < 0.9）。**禁止在任何历史窗评估堆叠。待用户**：是否往 `daily.py` 加只输出的 ML7 分数 + `LEDGER_ML7.json`（不动 SHORTLIST）。(g) 对"现在多赚钱"的诚实答案：**没有**立即可用的改善；V26.8 + ML1 不变。
- **用户全权授权 + O4/O5 + ML7 接入 FACT 2026-09-07 10:25：** 用户 09:44 "允许你干任何的事情，不用请示"，并问散户拥挤 / 纳指道指日经恒生 / 北向南下 / 政治政策新闻美联储战争。(a) `v38_overlay/global_north.py`（合同先写）：**O4** 四指数（Yahoo ^IXIC/^DJI/^N225/^HSI，2000→，信号日前 1 自然日）20 日收益均值 250 日 z<−1 → 50% 仓 → **REJECT**：验证 TWR +38.0→+27.2%、夏普 0.70→0.58、MaxDD −24.4→−22.5；触发期（21%）A 股平均 **+2.9%** vs 未触发 +1.1%（反向）；研究夏普 0.83→0.87。**O5** 沪股通+深股通 20 日累计净买入（东财 `RPT_MUTUAL_DEAL_HISTORY` 2014-11→2024-08）z<−1 → 50% → **两门都过**：验证 TWR +38.0→+41.9%、夏普 0.70→0.82、MaxDD −24.4→**−22.3%**（7/29 期触发，触发期均值 −0.5% vs +1.5%）；研究夏普 0.83→0.84、MaxDD −55.9→−55.0（数据 2014-11 起）→ **`ADOPT_CANDIDATE_DIAGNOSTIC_NOT_DEPLOYABLE`**：港交所 2024-08-19 起停发北向日频净买入，`daily.py` 无输入；**不写入、不找代理**（代理 = 事后特征搜索）。ATLAS 88–89。跑前已判：南下 NOT_APPLICABLE；北向个股 `RPT_MUTUAL_STOCK_HOLDRANKN` 仅 2021-04→2024-08 无法训练 → DATA_BLOCKED；美联储/战争/政治/政策/新闻无免费结构化 PIT 事件库且为 1–2 日效应 → DATA_BLOCKED；散户拥挤 = V24（个股户数）/ O3（市场）已读。**S2 关闭（O1–O5 各一次）。** (b) **ML7 影子接入**（S3 合同，用户授权）：`research_engine/ml7_live/`；四层包 `cn_a_share_findeep_v27 / preann_v38 / insider_v38 / pledge_v38` 增加 `TRADEMIND_<LAYER>_RAW|NORM|FEAT|*_CUTOFF|END_YEAR` env 覆盖（默认 = 冻结路径，冻结结果不变）；`live/info_layers/<layer>/raw` = 冻结原始文件硬链接 + 未关账周期（≤400 天）文件 >3 天重抓 + 各层可断点 `download.main()` 补到当年（首跑 20 分钟：季报 48 文件、预告 24、增减持 4、质押 2；后续只重抓当期）；编译 cutoff = asof（季报 415,157 事件、预告 114,399、增减持 313,220、质押 638 快照）；32 特征 → REFIT_240 同表 LightGBM（2025-11-06，2,446,408 行，增益 BP 18% / PL_NEG_MCAP 10% / PL_NEG_RATIO 9%）→ `SIGNAL_ML7_{date}.json`（¥5M 参考等权 999 只）+ `LEDGER_ML7.json|csv`（同 ML1 信号链，`name_overlap_with_ml1`）+ `STATUS_ML7.json`。`ml1_live/daily.py` 第 9 步在 ML1 全部文件写完后调用；`--no-ml7`；异常只记 `STATUS.json["ml7"]`。首信号 2026-08-28 与 ML1 前 20% 重合 **38.6%**。集成 `--skip-fetch` 运行：ML1 文件时间戳未变，ML7 2 秒。**读取规则**：`periods_until_read` 24 → 0 后读一次（超额 vs EW t ≥ 2、LO20 > 0、corr vs ML1 < 0.9）；之前任何数字不是结论；ML7 不进 SHORTLIST；不用 ML7 改 ML1。
- **V38 Evolution Mission 开启 FACT 2026-09-06 22:50（用户授权）：** 用户 22:37「你自己再回测、分析、优化、持续迭代……多看其他策略、论坛、文章……主动进化」；22:45 答复：允许 V38 长期研究（纸面路径已可用，V29 Stability 是日历等待）；外壳级市场仓位规则"你看着来"；数据尽量免费；问"韭菜/散户/羊群/心理指数"。**设计 `V38_EVOLUTION_MISSION_DESIGN.md`**：进化对象 = ML1 之外（**ML1 本身继续冻结**，对它"回测→优化→迭代"= ATLAS 81 行 + V34 已证只会让回测好看）。S0 外部学习 → `V38_HYPOTHESIS_LEDGER.md`（对照 ATLAS 去重，帖子只当想法不当证据）；S1 免费新信息层 L1 高管/股东增减持、L2 回购、L3 解禁（排除项）、L4 业绩预告/快报、L5 大宗交易、L6 基金持仓、L7 质押、L8 散户情绪复合（市场级 → 只作 S2 仓位输入，不作 CS 因子；仓库此前**没有**任何散户情绪指标）；每层一个预注册模型、LO20 事前账本、5 滚动窗、corr vs ML1 <0.90、m 逐层计。S2 组合层三条各测一次：O1 主板 EW < SMA200 → 仓位 50%；O2 20 日波动 > 研究期中位 ×1.5 → 50%；O3 情绪 > 90 分位 → 50%；采纳 = 验证 MaxDD 改善 **且** 夏普不低（两者同时）。S3 仅当出现独立袖，等权不优化。禁用窗不读；近期窗不读；不加个股出场规则；不换市场；不做 ML1 变体。每层完成即汇报 + ATLAS 行。V29 每日照跑。
- **用户 11:53 纠正 + V29 纸面路径 FACT 2026-09-06：** 用户：「你不是在搞 A 股吗，咋又换方向了……一个你还没搞好呢」。承认：V26.8 外壳冻了之后不该去跑 V31/V35–V37。期货四刀已冻、不是下一刀。已修：`LEDGER_TOP20` 记 2026-08-28 OPEN（入场 08-31，10 只，估 ¥19,746，V26.8）；`SHORTLIST_2026-09-04` 已重打成 V26.8（10 只，估 ¥19,629；09-04 不是换仓日，只是预览）。当前唯一工作 = `daily.py` + V26.8 短名单 + 影子账本。不改 ML1/V26.8。不发单。Stability 仍待连续交易日真实拉取 + 约 09-29 第一期结算。Paper 待用户开始日。
- **V37 仓单 FACT 2026-09-06 12:00：** 合同 `V37_WAREHOUSE_RECEIPT_CONTRACT.md` 先提交。上期所 `dailystock.dat` 直连 2,801 日、22 品种、5 个仓单特征、MIN_N=12。郑商所 xls 列漂移不纳入；大商所 412 不纳入。**`CN_FUTURES_WAREHOUSE_V37_NO_CANDIDATE`**：G1 验证 LS +41.4%（过）；G2 t 2.41（不过）；G3 滚动 3/5（不过）；G4 研究 −7.7%（不过）；G5 验证 corr vs ML1 0.41（不过）。验证同号、研究反号。禁止降 t 门、禁止看完 +41% 再加交易所。ATLAS 81。组合仍 1 袖。
- **V36 行业→期货 FACT 2026-09-06 11:40：** 合同 `V36_INDUSTRY_TO_FUTURES_CONTRACT.md` 先提交后跑。行业 as-of 止于 2024-02-15 → 研究 2019-07→2022-06 冻结、验证 2022-07→2024-02。5 个行业桶特征（每品种不同 X，不用 V31 的 8 个、不用 V35 的全市场指数）→ LightGBM → V31 同合约 LS 三分位。**`CN_INDUSTRY_TO_FUTURES_V36_NO_CANDIDATE`**：G1 验证 LS −11.6%（不过）；G2 t −0.89（不过）；G3 滚动 2/5（不过）；G4 研究 LS −30.7%（不过）；G5 验证 corr −0.25（过）。1/5。行业映射是反号噪声，不是弱残差。ATLAS 80。层关闭。组合仍 1 袖（ML1 V26.8）。禁止改映射/加 V31 特征/扩桶/把验证改回 2026/为小资金另做翻倍账本。
- **V35 A股→商品 FACT 2026-09-06 10:25：** 合同 `V35_CN_INFO_TO_COMMODITY_CONTRACT.md` 先提交后跑。CU/AG/AU/SC 同合约 20 日收益 + HS300/ZZ500 五个指数特征（不用 V31 的 8 个），篮子择时 LONGFLAT。指数文件止于 **2024-02-29**，验证只有 8 期。**`CN_INFO_TO_COMMODITY_V35_NO_CANDIDATE`**：G1/G3/G4 过；G2 t 1.86；G5 不过（验证期 frac_on=1，LF≡EW4）。研究期有择时（51% 在场）但 LONGFLAT +18% < 死拿 +36%。层关闭。ATLAS 79。对照图谱：**可立即做的合法新 D1 对象 = 0**。组合仍 1 袖（ML1 V26.8）。
