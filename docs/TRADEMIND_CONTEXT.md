...
- **V26.5 FACT 2026-09-06 00:25（用户：资金利用率允许 80%，必要时 100%）：** 前向合同 = V26.4 全部规则 + 总仓 80%（N=8 起）；`daily.py` 默认 `--exposure 0.80`；09-04 名单 8 只 ≈ ¥13,427。**不重读历史**（仓位影响是算术性的；再读就是禁止的"在历史上试仓位比例"）；历史依据继承 V26.4 的 70% 读取；事前预期验证期 ≈ +15%、MaxDD ≈ −9%、闲置 ≈ 42%。100% 不是系统规则，是用户手工加仓的判断；影子账本按 80% 记。`V26_5_ML1_EQMONEY_80PCT_MAIN_NOTE.md`。
